...
async def admin_stats_show(update, context):
    ...
    text = (
        ...
        f"✅ تایید‌شده: {orders.get('approved', 0)}\n"
...
async def admin_logs_show(update, context):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("👮 فعالیت ادمین‌ها", callback_data="logs_filter:admin")],
        [InlineKeyboardButton("❌ خطاهای ربات", callback_data="logs_filter:error")],
        [InlineKeyboardButton("🔌 لاگ مرزبان", callback_data="logs_filter:marzban_api")],
        [InlineKeyboardButton("📜 همه لاگ‌ها", callback_data="logs_filter:all")],
    ])
