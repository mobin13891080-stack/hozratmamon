...
async def get_groups(force_refresh=False):
    """گروه‌های پنل پاسارگارد را برمی‌گرداند (این پنل به‌جای inbound های خام
    مرزبان قدیمی، از مفهوم \"Group\" برای تعیین دسترسی کاربر استفاده می‌کند)."""
    ...
    url = config.PANEL_URL + "/api/groups"
...
async def get_active_group_ids():
    """شناسه تمام گروه‌های غیرغیرفعال پنل - برای ساخت کاربر جدید با دسترسی
    کامل به همه گروه‌ها (معادل رفتار قبلی که کاربر به همه inbound ها دسترسی داشت)."""
    groups = await get_groups()
    return [g["id"] for g in groups if not g.get("is_disabled")]
