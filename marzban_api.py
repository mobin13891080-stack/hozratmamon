...
# هر درخواست مهم در جدول logs دیتابیس ثبت می‌شود (لاگ API مرزبان)
...
async def create_user(gb, days, username=None):
    token = await get_token()
...
async def modify_user(username, **fields):
    ...
    headers = {"Authorization": "Bearer " + token}
    async with httpx.AsyncClient(timeout=20) as client:
        response = await client.put(url, headers=headers, json=fields)
        if response.status_code >= 400:
...
async def revoke_sub(username):
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        ...
        if response.status_code >= 400:
            ...
            raise Exception("خطا در تفییر لینک اشتراک (" + str(response.status_code) + "): " + response.text)
