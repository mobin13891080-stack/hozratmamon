...
ASYNC_SETTING_ACTIONS = {
    ...
    "admin_set_forcejoin_channel": ("force_join_channel", "آیدی/یوزرنیم کانال را وارد کنید (مثلا @mychannel):"),
...
def main_menu_keyboard(is_admin_user):
    rows = [[BTN_BUY, BTN_TRIAL], [BTN_ACCOUNT, BTN_SUPPORT]]
    if is_admin_user:
...
def admin_menu_keyboard(perms, bot_enabled):
    ...
    if "bot_toggle" in perms:
        rows.append([BTN_ADMIN_TURN_OFF if bot_enabled else BTN_ADMIN_TURN_ON])
...
async def show_join_gate(chat_id, context, channel):
    ...
    await context.bot.send_message(chat_id=chat_id, text="برای استفاده از ربات، ابتدا در کانال زیر عضو شوید:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def start(update, context):
    ...
    if force_join == "1":
        ...
        if channel and not await check_membership(context, channel, user.id):
...
async def check_join_callback(update, context):
    ...
    channel = await database.get_setting("force_join_channel", "")
    if await check_membership(context, channel, query.from_user.id):
...
async def buy_show_packages(update, context):
    ...
    keyboard = [[InlineKeyboardButton(
        f"📦 {p['name']} — {p['gb']}گیگ/{p['days']}روز — {p['price']:,}ت", callback_data=f"pkg:{p['id']}"
    )] for p in packages]
...
async def select_package(update, context):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎟️ کد تخفیف دارم", callback_data="discount_have")],
        [InlineKeyboardButton("✅ ادامه بدون تخفیف", callback_data="discount_skip")],
    ])
...
async def discount_have_callback(update, context):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("کد تخفیف را وارد کنید: (برای رد کردن /skip بزنید)")
