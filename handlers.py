...
async def safe_edit_or_send(query, text, reply_markup=None, parse_mode="HTML"):
    """ویرایش امن پیام فعلی (منو) به‌جای فرستادن پیام جدید و شلوغ کردن چت.
...
async def show_error_with_back(update, context, error_text):
    ...
    به‌جای باقی ماندن منوی قبلی و فرستادن پیام خطای جدا و بی‌ربط."""
...
async def require_perm(query, user_id, permission):
    if await database.admin_has_permission(user_id, permission):
        return True
    await query.answer("⛔ شما دسترسی به این بخش را ندا��ید.", show_alert=True)
    return False
...
async def check_membership(context, channel, user_id):
    try:
        ...
        # دلیل این خطا معمولا اینه که ربات در کانال ادمین نیست. در لاگ ثبت می‌شود تا ادمین متوجه شود
        try:
            await database.log_action(
...
async def _post_trust_channel(context, buyer_user, service_username, package, final_price, expire_ts):
    try:
        ...
        try:
            await database.log_action("error", None, f"خطا در ارسال به کانال ��عتماد: {error}")
        except Exception:
            pass
...
async def help_show(update, context):
    ...
    text = (
        ...
        "بعد از کارت به کارت، فقط کافیست ع����س رسید واریز را ارسال کنید.\n"
...
async def _build_account_detail(order):
    ...
    if is_trial:
        action_rows.append([ibtn("⚙️ تغییر لینک", callback_data=f"acc_revoke:{order_id}")])
    else:
        action_rows.append([ibtn("💊 تمدید سرویس", callback_data=f"pkg:{package.get('id', 0)}", style="success"), ibtn("⚙️ تغییر لینک", callback_data=f"acc_revoke:{order_id}")])
...
async def account_revoke_callback(update, context):
    ...
    text, keyboard = await _build_account_detail(order)
    text = "✅ <b>لینک اشتراک با موفقیت تغییر شد و دسترسی دستگاه‌های قبلی قطع شد.</b>\n\n" + text
    await safe_edit_or_send(query, text, reply_markup=keyboard)
...
async def wallet_topup_start(update, context):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("💳 <b>مبلغی که می‌خواهید به کیف پول اضافه شود را به تومان وارد کنید:</b>", parse_mode="HTML", reply_markup=flow_cancel_keyboard())
...
async def pay_with_wallet_callback(update, context):
    ...
    if not final_price or not package_id:
        ...
        return ConversationHandler.END
    spent = await database.try_spend_wallet_balance(user_id, final_price)
    if not spent:
