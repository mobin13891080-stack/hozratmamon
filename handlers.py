...
async def _prompt_desired_username(update_or_query):
    ...
    text = (
        "📝 <b>یک نام کاربری دلخواه ارسال کنید</b>\n"
        "⚠️ نام کاربری باید بدون کاراکترهای اضافه مانند @ ، فاصله ، خط تیره باشد.\n"
        "⚠️ نام کاربری باید انگلیسی باشد (فقط حروف و اعداد، شروع با حرف).\n"
        "👇👇\n"
        "✅ نام کاربری های صحیح  : <code>mmd11</code> | <code>mahdi</code> | <code>ws1ksdf</code>\n"
        "👆👆\n"
        "❌ نام کاربری های نادرست : <code>ali_</code> | <code>tele@</code> | <code>_mahdi</code> | <code>محسن</code> | <code>ws1_ksdf</code>"
    )
...
async def receive_desired_username(update, context):
    ...
    if not _validate_desired_username(text):
        ...
        return USERNAME_INPUT
    context.user_data["desired_username"] = text.lower()
