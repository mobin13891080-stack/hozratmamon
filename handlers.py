...
async def check_membership(context, channel, user_id):
    try:
        ...
        # امنیت رفرال/جوین اجباری: Timeout هرگز به معنی «عضو است» نیست.
        # کاربر پیام retry می‌گیرد و ربات بدون ثبت رفرال یا عبور جعلی ادامه نمی‌دهد.
        return False
        ...
        try:
            ...
            pass
        # خطای نامشخص نیز fail-closed است؛ عبور جعلی می‌تواند پاداش رفرال ایجاد کند.
        return False
...
# کش کوتاه‌مدت برای نتیجه force_join — فقط نتیجه PASS (همه عضو) کش می‌شود
# نتیجه FAIL کش نمی‌شود تا دسترسی کاذب ایجاد نشود
# ۶۰ ثانیه کش باعث می‌شود هر کلیک دکمه دیگر ۲ API call به تلگرام نزند → رفع freeze
import time as _time
_join_cache: dict = {}  # {user_id: (timestamp, result_was_empty)}
_JOIN_CACHE_TTL = 60  # ثانیه — فقط برای نتیجه موفق (عضو همه کانال‌ها)


async def get_missing_channels(context, user_id):
    """عضویت فعلی کاربر را از Telegram می‌گیرد.
    نتیجه عضویت کامل (= [] ) برای ۶۰ ثانیه کش می‌شود تا هر کلیک دکمه ۲ API call
    به تلگرام نزند و دکمه‌ها freeze نکنند. نتیجه رد (کانال باقی‌مانده) هرگز کش نمی‌شود."""
    ...
    # کش فقط برای نتیجه موفق
    cached = _join_cache.get(user_id)
    if cached:
        ts, was_ok = cached
        if was_ok and (_time.monotonic() - ts) < _JOIN_CACHE_TTL:
            return []  # کاربر ۶۰ ثانیه پیش تأیید شده بود

    # تمام کانال‌ها هم‌زمان چک می‌شوند
    ...
    # فقط نتیجه موفق (= عضو همه) رو کش کن
    if not missing:
        _join_cache[user_id] = (_time.monotonic(), True)
...
async def check_join_callback(update, context):
    query = update.callback_query
    # این callback همیشه کش را پاک می‌کند تا بررسی واقعی از Telegram انجام دهد
    _join_cache.pop(query.from_user.id, None)
