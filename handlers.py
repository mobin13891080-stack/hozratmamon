...
async def send_main_menu(chat_id, context, user_id, account_name=None):
    ...
    if not referral_enabled or (is_admin_user and user_id != config.ADMIN_ID):
...
async def start(update, context):
    ...
    # لینک تست رفرال نسخه ۴: این لینک عمداً وارد سیستم واقعی referral می‌شود،
    # اما با is_test=1 از آمار و attribution واقعی جدا می‌ماند تا ادمین بتواند
    # ثبت پاداش، شرط آزادسازی و خرید واجدشرایط را واقعاً تست کند.
    if context.args:
        raw_arg = str(context.args[0]).strip()
        if raw_arg.startswith("testref_"):
            token = raw_arg[len("testref_"):].strip()
            test_referral = await database.apply_referral_test_start(user.id, token)
            if test_referral:
                amount = int(test_referral.get("reward_amount") or 0)
                status = test_referral.get("status") or "locked"
                status_text = (
                    f"🔒 پاداش {amount:,} تومان ثبت شد و فعلاً قفل است؛ شرط آزادسازی را می‌توانی با خرید اکانت تست کنی."
                    if status == "locked" else
                    f"🔓 پاداش {amount:,} تومان فوراً قابل استفاده شد."
                )
                await database.log_action("referral_test", test_referral["test_owner_id"],
                                          f"ثبت تست رفرال #{test_referral['id']} با اکانت {user.id}")
                try:
                    await context.bot.send_message(
                        chat_id=test_referral["test_owner_id"],
                        text=(f"🧪 <b>دعوت تستی ثبت شد</b>\n👤 اکانت تست: <code>{user.id}</code>\n"
                              f"🆔 رکورد: <code>#{test_referral['id']}</code>\n💰 پاداش: <b>{amount:,} تومان</b>\n{status_text}"),
                        parse_mode="HTML",
                    )
                except Exception:
                    pass
                await update.effective_message.reply_text(
                    "🧪 تست رفرال ثبت شد و اطلاعات آن برای ادمین ثبت گردید.",
                    parse_mode="HTML",
                )
                # تست نباید قوانین/رفرال واقعی را دوباره به کاربر وصل کند؛ از اینجا
                # مسیر عادی /start ادامه پیدا می‌کند تا همان اکانت قابل استفاده بماند.
    # دریافت پارامتر لینک دعوت؛ ثبت نهایی تا زمانی که کاربر قوانین/جوین اجباری را کامل نکرده انجام نمی‌شود.
    if context.args:
        referral_arg = str(context.args[0]).strip()
        if referral_arg.startswith("ref_"):
            try:
                referrer_id = int(referral_arg[4:])
                if referrer_id == user.id and await database.get_setting("referral_enabled", "0") == "1":
                    await update.effective_message.reply_text("❌ <b>نمی‌تونی با لینک دعوت خودت وارد ربات بشی.</b>", parse_mode="HTML")
                    return
                if referrer_id != user.id and await database.get_setting("referral_enabled", "0") == "1":
...
async def wallet_topup_approve_callback(update, context):
    ...
    released_by_topup = 0
    try:
        released_by_topup = await database.release_referral_by_real_topup(tx["user_id"], tx["amount"])
