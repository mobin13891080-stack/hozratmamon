...
_DANGER_HINTS = ("❌", "🗑", "🔴", "🚫", "حذف", " رد", "رد ", "لفو", "مسدود", "خاموش", "بستن", "حذف")
...
def kbtn(text, style="primary"):
    """ساخت دکمه رنگی منوی ثابت (ReplyKeyboard). اگر style=None باشد، دکمه بدون رنگ (پیش‌فرض تلگرام) ساخته می‌شود."""
    if style is None:
...
BTN_ADMIN_PACKAGES = "📦 محیریت پلن‌ها"
...
def _mask_user_id(user_id):
    """ماسک کردن ایدی تلگرام برای نمایش عمومی در کانال اعتماد، متلا 65*278"""
    s = str(user_id)
    if len(s) <= 5:
...
ASYNC_SETTING_ACTIONS = {
    ...
    "admin_set_support_username": ("support_username", "آیدی پشتیبانی را وارد کنید (متلا @sup):"),
...
def main_menu_keyboard(is_admin_user):
    rows = [
        [kbtn(BTN_BUY, style="success")],
        [kbtn(BTN_TRIAL, style="danger")],
        [kbtn(BTN_WALLET, style=None), kbtn(BTN_ACCOUNT, style=None)],
...
async def _show_payment_choice(update_or_query, context, final_price, package=None, show_discount_option=True):
    ...
    lines.append(f"💵 <b>مبلف قابل پرداخت:</b> {final_price:,} تومان")
...
async def trial_run(update, context):
    ...
    try:
        ...
        await database.update_order(order_id, status="approved", marzban_username=user_data.get("username", ""), subscription_link=link)
...
def _account_list_text_and_keyboard(approved):
    text = (
        "📱 <b>سرویس‌های من</b>\n"
        "━━━━━━━━━━━\n"
        "در ادامه لیست سرویس‌های خریداری‌شده و تست رایگان فع��ل شما آمده است.\n"
...
async def wallet_topup_start(update, context):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("💳 <b>مبلفی که می‌خواهید به کیف پول افزوده شود را به تومان وارد کنید:</b>", parse_mode="HTML", reply_markup=flow_cancel_keyboard())
...
async def wallet_topup_cancel(update, context):
    context.user_data.pop("wallet_topup_amount", None)
    await update.message.reply_text("شارژ کیف پول لفو شد.")
    return ConversationHandler.END
...
async def wallet_topup_approve_callback(update, context):
    ...
    try:
        await context.bot.send_message(
            chat_id=tx["user_id"],
            text=f"✅ شارژ کیف پول شما به مبلف {tx['amount']:,} تومان تایید شد. 🎉\n💰 موجودی جدید: {new_balance:,} تومان",
        )
    except Exception:
        pass
...
async def wallet_topup_reject_callback(update, context):
    ...
    try:
        await context.bot.send_message(chat_id=tx["user_id"], text="❌ متاسفانه رسید شارژ کیف پول شما تایید نشد. با پشتیبانی در ارطباط باشید.")
    except Exception:
        pass
...
async def pay_with_wallet_callback(update, context):
    ...
    discount_code = context.user_data.get("discount_code")
    balance = await database.get_wallet_balance(query.from_user.id)
    if balance < final_price:
        await query.answer()
        await query.message.reply_text(
            f"⚠️ <b>موجودی کیف پول کافی نیست.</b>\n💰 موجودی فعلی: {balance:,} تومان\n🧾 مبلف مورد نیاز: {final_price:,} تومان\n\nلطفا ابتدا کیف پولتان را از منوی «کیف پول» شارژ کنید.",
            parse_mode="HTML",
        )
        return ConversationHandler.END
    await query.answer("⏳ در حال پردازش...")
    order_id = await database.create_order(
        query.from_user.id, package_id, None, price=final_price, discount_code=discount_code,
        order_type="purchase", custom_gb=custom_gb, custom_days=custom_days,
    )
    await database.adjust_wallet_balance(query.from_user.id, -final_price)
    await database.add_wallet_transaction(query.from_user.id, -final_price, "purchase", related_order_id=order_id)
    order = await database.get_order(order_id)
    try:
        await _fulfill_order(order, context)
        await query.message.reply_text("✅ <b>خرید با موفقیت از کیف پول انجام شد و سرویس فعال گردید!</b>", parse_mode="HTML")
        await database.log_action("purchase", query.from_user.id, f"خرید با کیف پول #{order_id}")
    except Exception as error:
        await database.adjust_wallet_balance(query.from_user.id, final_price)
        await database.add_wallet_transaction(query.from_user.id, final_price, "admin_credit", related_order_id=order_id)
        await database.update_order(order_id, status="rejected")
        await query.message.reply_text(
            f"❌ <b>متاسفانه در ساخت سرویس خطایی رخ داد، مبلف به کیف پول شما برگشت.</b>\nخطا: <code>{html.escape(str(error))}</code>",
            parse_mode="HTML",
        )
        try:
