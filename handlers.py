...
async def _apply_referral_penalty(context, referral, channel):
    ...
    if (referral.get("penalty_status") or "none") == "done":
        return False
    rid = int(referral["id"])
...
async def scan_referral_penalties(context):
    ...
    async with _referral_penalty_scan_lock:
        try:
            ...
            if not channels:
                return
            candidates = await database.list_referral_penalty_candidates(limit=50)
            sem = asyncio.Semaphore(8)
            async def one(referral):
                async with sem:
                    if referral.get("left_at") and (referral.get("penalty_status") or "none") == "pending":
                        await _apply_referral_penalty(context, referral, referral.get("left_channel") or "-")
                        return
                    checks = await asyncio.gather(*[check_membership_strict(context, c, referral["referred_id"]) for c in channels])
                    for c, (is_member, _) in zip(channels, checks):
                        if is_member is False:
                            claimed = await database.claim_referral_leave(referral["id"], c)
                            if claimed and (claimed.get("penalty_status") or "pending") == "pending":
                                await _apply_referral_penalty(context, claimed, c)
                            return
                    await database.mark_referral_penalty_checked(referral["id"])
            await asyncio.gather(*(one(r) for r in candidates), return_exceptions=True)
        except Exception:
            logger.exception("خطا در پایش جریمه رفرال")
...
async def start(update, context):
    ...
    # دریافت پارامتر لینک دعوت؛ V10 از کوتاه‌ترین payload یعنی ID عددی استفاده می‌کند و لینک‌های قدیمی نیز معتبر می‌مانند.
    if context.args and referral_enabled == "1":
        referral_arg = str(context.args[0]).strip()
        referrer_id = database.resolve_referral_code(referral_arg)
