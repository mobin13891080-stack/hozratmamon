...
async def safe_edit_or_send(query, text, reply_markup=None, parse_mode="HTML"):
    ...
    try:
        ...
        if "not modified" in err:
            return
        # پیام حذف شده یا پیدا نشد → ارسال پیام جدید
        if "message to edit not found" in err or "bad request" in err or "message_id_invalid" in err:
            pass  # ادامه به ارسال پیام جدید
        else:
            pass  # برای هر خطای دیگری هم ارسال پیام جدید امن‌تر است
    try:
...
async def safe_edit_text(query, text, reply_markup=None, parse_mode="HTML"):
    """ویرایش امن متن پیام: اگر «not modified» باشد نادیده می‌گیریم.
    اگر پیام حذف شده (Message to edit not found) پیام جدید ارسال می‌کنیم."""
    try:
        await query.edit_message_text(text, parse_mode=parse_mode, reply_markup=reply_markup)
    except Exception as e:
        err = str(e).lower()
...
async def _safe_edit_caption(query, caption, parse_mode="HTML"):
    """ویرایش امن کپشن: خطاهای not modified و message not found هر دو مدیریت می‌شوند."""
    try:
        ...
        if "not modified" in err:
            return
        if "message to edit not found" in err or "message_id_invalid" in err:
            return  # پیام حذف شده؛ نادیده بگیر
        raise


async def _safe_edit_caption_or_text(query, suffix_or_text, is_suffix=True, parse_mode="HTML"):
    """نسخه مقاوم‌تر ادیت: تشخیص caption vs text و مدیریت خطاهای message not found."""
    ...
    try:
        ...
        if "not modified" in err:
            return
        if "message to edit not found" in err or "message_id_invalid" in err:
            return  # پیام قدیمی/حذف‌شده؛ نادیده بگیر
        raise
...
BTN_REFERRAL = "👥 دعوت دوستان و کسب درآمد"
BTN_HELP = "❓ راهنما"
...
BTN_ADMIN_PASARGUARD = "🛡 ابزار پاسارگارد"
BTN_ADMIN_BULK_BONUS = "🎁 هدیه همگانی"
...
BTN_ADMIN_REFERRAL = "👥 رفرال‌گیری"
BTN_ADMIN_RULES = "📜 قوانین و لینک‌ها"
BTN_ADMIN_GUIDE = "📚 آموزش بعد از تایید رسید"
...
BTN_ADMIN_PKG_TEMPLATE = "🎨 قالب نمایش پلن‌ها"
BTN_ADMIN_BUY_PROMPT = "💬 متن انتخاب پلن"
...
BTN_ADMIN_CONNECTION_GUIDE_SETTINGS = "📲 راهنمای کاربران (اتصال)"
BTN_ADMIN_DEBUG = "🐛 پنل دیباگ"
BTN_ADMIN_SERVICE_TEMPLATE = "🎨 قالب سرویس کاربران"
BTN_ADMIN_TRUST_CHANNEL = "📣 کانال اعتماد"

# TRUST_CHANNEL حذف شد - آیدی کانال از پنل مدیریت (trust_channel_id) خوانده می‌شود
...
BUTTON_PERMISSION = {
    ...
    BTN_ADMIN_DISCOUNTS: "discounts", BTN_ADMIN_CUSTOMBUILDER: "packages", BTN_ADMIN_SERVERS: "servers", BTN_ADMIN_PASARGUARD: "servers", BTN_ADMIN_BULK_BONUS: "orders", BTN_ADMIN_FINANCE: "finance",
    ...
    BTN_ADMIN_FORCEJOIN: "settings", BTN_ADMIN_TRIAL: "settings", BTN_ADMIN_REFERRAL: "settings", BTN_ADMIN_GUIDE: "settings", BTN_ADMIN_RULES: "settings",
    ...
    BTN_ADMIN_PKG_TEMPLATE: "packages", BTN_ADMIN_BUY_PROMPT: "settings",
    BTN_ADMIN_CONNECTION_GUIDE_SETTINGS: "settings", BTN_ADMIN_SERVICE_TEMPLATE: "settings", BTN_ADMIN_TRUST_CHANNEL: "settings",
    BTN_ADMIN_DEBUG: "logs",
}
...
ADMIN_MENU_ROWS = [
    ...
    [BTN_ADMIN_SERVERS, BTN_ADMIN_PASARGUARD],
    [BTN_ADMIN_BULK_BONUS],
    [BTN_ADMIN_FINANCE],
    [BTN_ADMIN_NOTIFY, BTN_ADMIN_TICKETS],
    [BTN_ADMIN_STATS, BTN_ADMIN_ADMINS],
    [BTN_ADMIN_LOGS],
    [BTN_ADMIN_CARD, BTN_ADMIN_FORCEJOIN],
    [BTN_ADMIN_TRIAL, BTN_ADMIN_REFERRAL],
    [BTN_ADMIN_GUIDE],
    [BTN_ADMIN_RULES],
    [BTN_ADMIN_WELCOME, BTN_ADMIN_WELCOME_PHOTO],
    [BTN_ADMIN_WELCOME_GIF],
    [BTN_ADMIN_SUPPORT_SETTINGS],
    [BTN_ADMIN_PKG_TEMPLATE, BTN_ADMIN_BUY_PROMPT],
    [BTN_ADMIN_SERVICE_TEMPLATE, BTN_ADMIN_TRUST_CHANNEL],
    [BTN_ADMIN_CONNECTION_GUIDE_SETTINGS],
    [BTN_ADMIN_DEBUG],
]
...
ASYNC_SETTING_ACTIONS = {
    ...
    "admin_set_buy_prompt_text": (
        "buy_prompt_text",
        "✏️ متن جدید بالای لیست پلن‌ها را وارد کنید (بدون تگ HTML؛ به صورت خودکار بولد نمایش داده می‌شود):",
    ),
    "admin_set_payment_random_min": ("payment_random_min", "حداقل عدد رندوم اضافه‌شونده به مبلغ را به تومان وارد کنید:"),
    "admin_set_payment_random_max": ("payment_random_max", "حداکثر عدد رندوم اضافه‌شونده به مبلغ را به تومان وارد کنید:"),
    "admin_set_referral_reward": ("referral_reward", "مبلغ پاداش هر دعوت موفق را به تومان وارد کنید:"),
    "admin_set_referral_penalty_extra_money": ("referral_penalty_extra_money", "مبلغ اضافه جریمه هر لفت را به تومان وارد کنید (مثلا 2000):"),
    "admin_set_referral_penalty_gb": ("referral_penalty_gb", "در هر جریمه، چند گیگ از سرویس کم شود؟ (0 یعنی بدون جریمه حجمی):"),
    "admin_set_referral_penalty_days": ("referral_penalty_days", "در هر جریمه، چند روز از سرویس کم شود؟ (0 یعنی بدون جریمه زمانی):"),
    "admin_set_referral_penalty_text": (
        "referral_penalty_text",
        "متن پیام جریمه لفت را وارد کنید. متغیرها: {name} {user_id} {username} {channel} {penalty_details}"
    ),
    "admin_set_referral_success_text": ("referral_success_text", "متن پیام دعوت موفق را وارد کنید. متغیرها: {name} {user_id} {amount} {status_text}"),
    "admin_set_referral_page_text": ("referral_page_text", "متن صفحه رفرال را وارد کنید. متغیرها: {reward} {link} {code} {total} {successful} {unlocked} {locked} {condition_text}"),
    "admin_set_guide_text": ("post_approval_guide_text", "📚 متن آموزش بعد از تایید رسید را وارد کنید:"),
    "admin_set_guide_url": ("post_approval_guide_url", "لینک دکمه آموزش را وارد کنید؛ اگر نمی‌خواهید لینک باشد، 0 بفرستید:"),
}


def main_menu_keyboard(is_admin_user, show_connection_guide=False):
    rows = [
        [kbtn(BTN_BUY, style="success"), kbtn(BTN_TRIAL, style="danger")],
        [kbtn(BTN_ACCOUNT, style=None), kbtn(BTN_WALLET, style=None)],
        [kbtn(BTN_REFERRAL, style=None)],
        [kbtn(BTN_SUPPORT, style=None), kbtn(BTN_HELP, style=None)],
    ]
    if show_connection_guide:
        rows.append([kbtn(BTN_CONNECTION_GUIDE, style=None)])
    if is_admin_user:
...
async def send_main_menu(chat_id, context, user_id, account_name=None):
    # موازی‌سازی کامل: settings و is_admin با هم فتچ می‌شوند (یک رفت‌وبرگشت به‌جای دو تا)
    menu_settings, is_admin_user = await asyncio.gather(
        database.get_settings_many(
            ["welcome_text", "welcome_photo_id", "welcome_gif_id", "connection_guide_enabled", "referral_enabled"],
            {"welcome_text": "خوش امدید", "welcome_photo_id": "", "welcome_gif_id": "", "connection_guide_enabled": "0", "referral_enabled": "0"},
        ),
        database.is_admin(user_id),
    )
    welcome_text = menu_settings.get("welcome_text") or "خوش امدید"
    ...
    photo_id = menu_settings.get("welcome_photo_id") or ""
    gif_id = menu_settings.get("welcome_gif_id") or ""
    guide_value = menu_settings.get("connection_guide_enabled") or "0"
    referral_value = menu_settings.get("referral_enabled") or "0"
    show_guide = guide_value == "1"
    referral_enabled = referral_value == "1"
    keyboard = main_menu_keyboard(is_admin_user, show_connection_guide=show_guide)
    if not referral_enabled or is_admin_user:
        # ReplyKeyboardMarkup rows may be tuples; never mutate them in-place.
        # Rebuild the keyboard so the existing menu remains compatible with
        # python-telegram-bot versions that expose immutable row containers.
        filtered_rows = []
        for row in keyboard.keyboard:
            filtered_rows.append(
                [button for button in row if getattr(button, "text", "") != BTN_REFERRAL]
            )
        keyboard = ReplyKeyboardMarkup(filtered_rows, resize_keyboard=True)
    if gif_id:
        try:
...
async def check_membership(context, channel, user_id):
    """بررسی سریع عضویت با نتیجه سه‌حالته: True=عضو، False=قطعاً غیرعضو، None=نامشخص.

    timeout/خطای Telegram دیگر به‌اشتباه «غیرعضو» حساب نمی‌شود؛ چون همین اشتباه در outage
    Telegram می‌توانست Force-Join را برای همه فعال کند و کل منوی ربات از دید کاربر فریز شود.
    بررسی‌های سخت‌گیرانه مربوط به جریمه رفرال همچنان در check_membership_strict باقی مانده‌اند.
    """
    try:
        member = await asyncio.wait_for(
            context.bot.get_chat_member(chat_id=channel, user_id=user_id), timeout=2.0
        )
        return member.status in ("member", "administrator", "creator")
    except asyncio.TimeoutError:
        logger.warning("forcejoin membership timeout channel=%s user=%s", channel, user_id)
        return None
    except Exception as error:
        logger.warning(
            "forcejoin membership check failed channel=%s user=%s error=%s: %s",
            channel, user_id, type(error).__name__, error,
        )
        return None


async def check_membership_strict(context, channel, user_id):
    """برای جریمه فقط نتیجه قطعی خروج را قبول می‌کند؛ خطا/timeout هرگز جریمه ایجاد نمی‌کند."""
    try:
        member = await asyncio.wait_for(context.bot.get_chat_member(chat_id=channel, user_id=user_id), timeout=4.0)
        return member.status in ("member", "administrator", "creator"), str(member.status or "")
    except Exception as exc:
        return None, type(exc).__name__


async def get_missing_channels(context, user_id):
    """بررسی تازه و هم‌زمان عضویت کاربر در همه کانال‌های اجباری.

    نتیجه موفق عمداً cache نمی‌شود؛ چون کاربر ممکن است بلافاصله بعد از تأیید از
    کانال خارج شود و نباید تا چند ده ثانیه دسترسی سرویس‌ها را نگه دارد.
    """
    enabled, channels = await asyncio.gather(
        database.get_setting("force_join_enabled", "0"),
        get_force_join_channels(),
    )
    if enabled != "1" or not channels:
        return []

    results = await asyncio.gather(
        *(check_membership(context, channel, user_id) for channel in channels),
        return_exceptions=False,
    )
    # فقط پاسخ قطعی Telegram مبنی بر «غیرعضو» باید دسترسی را متوقف کند.
    # None یعنی Telegram در آن لحظه پاسخ قطعی نداده؛ در این حالت UX نباید قفل شود.
    return [channel for channel, is_member in zip(channels, results) if is_member is False]


def _forcejoin_channel_url(channel):
...
def _forcejoin_channel_label(channel):
    raw = str(channel).strip()
    if raw.startswith("https://t.me/"):
        raw = raw.split("?", 1)[0].rstrip("/").rsplit("/", 1)[-1]
    return raw if raw.startswith("@") else "@" + raw.lstrip("@")


async def show_join_gate(chat_id, context, channels):
    """صفحهٔ تمیز جوین اجباری؛ فقط کانال‌های باقی‌مانده نمایش داده می‌شوند."""
    keyboard = []
    for channel in channels:
        label = _forcejoin_channel_label(channel)
        keyboard.append([ibtn(f"📢 عضویت در {label}", url=_forcejoin_channel_url(channel), style="primary")])
    keyboard.append([ibtn("✅ بررسی عضویت من", callback_data="check_join", style="success")])
    count = len(channels)
    if count == 1:
        text = (
            "🔒 <b>برای استفاده از ربات، عضویت کامل الزامی است.</b>\n\n"
            "هنوز در این کانال عضو نشده‌ای: \"" + _forcejoin_channel_label(channels[0]) + "\"\n\n"
            "ابتدا عضو شو، سپس روی «✅ بررسی عضویت من» بزن."
        )
    else:
        names = "\n".join(f"• {_forcejoin_channel_label(c)}" for c in channels)
        text = (
            "🔒 <b>برای استفاده از ربات، باید در همهٔ کانال‌های زیر عضو باشی.</b>\n\n"
            f"📌 <b>{count} کانال باقی مانده:</b>\n{names}\n\n"
            "بعد از عضویت در همه، روی «✅ بررسی عضویت من» بزن."
        )
...
async def _finalize_referral_after_gate(chat_id, context, user_id):
    """ثبت رفرال فقط بعد از عبور کامل از قوانین و جوین اجباری و ارسال پیام نتیجه به معرف."""
    try:
        referral = await database.finalize_pending_referral(user_id)
        if not referral:
            return
        referrer_id = referral["referrer_id"]
        amount = int(referral["reward_amount"] or 0)
        condition_enabled = await database.get_setting("referral_condition_enabled", "1") == "1"
        if condition_enabled:
            status_text = (
                "🔒 این پاداش فعلاً در انتظار خرید واجدشرایط دوست شماست.\n"
                "پس از اینکه تمام دعوت‌های موفق شما خرید واجدشرایط انجام دهند، مجموع پاداش‌های در انتظار آزاد و قابل استفاده می‌شود."
            )
        else:
            status_text = "🔓 پاداش شما همین حالا آزاد شد و می‌توانید از آن برای خرید استفاده کنید."
        try:
            invited_chat = await context.bot.get_chat(user_id)
            invited_name = invited_chat.full_name or invited_chat.first_name or invited_chat.username or str(user_id)
        except Exception:
            invited_name = str(user_id)
        ...
        try:
            ...
            text = (
                f"🎉 <b>دعوت موفق!</b>\n\n👤 نام: {html.escape(invited_name)}\n"
                f"🆔 آیدی: <code>{user_id}</code>\n💰 پاداش: <b>{amount:,} تومان</b>\n{status_text}"
            )
        try:
            await context.bot.send_message(chat_id=referrer_id, text=text, parse_mode="HTML")
        except Exception:
            pass
        await database.log_action("referral", referrer_id, f"دعوت موفق کاربر {user_id} | پاداش {amount} | وضعیت {referral['status']}")
    except Exception as exc:
        await database.log_action("error", user_id, f"خطا در تکمیل رفرال: {type(exc).__name__}: {exc}")
...
async def _proceed_after_rules(chat_id, context, user_id):
    if not await database.is_admin(user_id):
        ...
        if missing:
            ...
            return
    await _finalize_referral_after_gate(chat_id, context, user_id)
...
async def _apply_referral_penalty(context, referral, channel):
    """اعمال یک‌باره جریمه خروج؛ فقط منابعی که واقعاً از رفرال آمده‌اند هدف هستند.
    پول واقعی و سهم واقعی سرویس محافظت می‌شود."""
    rid = int(referral["id"])
    if await database.get_setting("referral_penalty_enabled", "0") != "1":
        return False

    reward = max(0, int(referral.get("reward_amount") or 0))
    extra = max(0, int(float(await database.get_setting("referral_penalty_extra_money", "0") or 0)))
    cash_target = reward + extra
    penalty_gb = max(0.0, float(await database.get_setting("referral_penalty_gb", "0") or 0))
    penalty_days = max(0, int(float(await database.get_setting("referral_penalty_days", "0") or 0)))

    # اول هر مقدار ممکن از خود منابع رفرالی؛ شامل بخش قفل‌شده هم می‌شود.
    cash_paid = await database.apply_referral_cash_penalty(referral["referrer_id"], cash_target)
    remaining_cash = max(0, cash_target - cash_paid)
    ...
    # اگر مبلغ جریمه نقدی باقی مانده باشد، فقط «سهم خریداری‌شده با رفرال» از سرویس‌ها هدف است.
    # سهمی که با کارت/شارژ واقعی خریده شده، هرگز دستکاری نمی‌شود.
    if remaining_cash > 0 and (penalty_gb > 0 or penalty_days > 0):
        orders = await database.get_active_referral_funded_orders(referral["referrer_id"])
        for order in orders:
            ...
            try:
                ...
                expire = marzban_api.parse_expire_timestamp(info.get("expire")) if hasattr(marzban_api, "parse_expire_timestamp") else int(info.get("expire") or 0)
                data_limit = max(0, int(info.get("data_limit") or 0))
                used = max(0, int(info.get("used_traffic") or 0))
                fields = {}
                funded_limit = max(0, int(order.get("referral_funded_data_limit") or 0))
                funded_days = max(0.0, float(order.get("referral_funded_days") or 0))
                base_limit = max(0, int(order.get("referral_base_data_limit") or 0))
                base_expire = max(0, int(order.get("referral_base_expire") or 0))

                if penalty_gb > 0 and funded_limit > 0 and data_limit > 0:
                    # فقط حجم منتسب به پول رفرالی قابل جریمه است. مصرف‌شده و سهم واقعی محفوظ می‌ماند.
                    requested_bytes = int(penalty_gb * marzban_api.GB)
                    reducible = min(requested_bytes, funded_limit)
                    # اگر پنل حجم را بعداً با پول واقعی افزایش داده، سقف کاهش همچنان funded_limit است.
                    target_limit = max(used, data_limit - reducible)
                    if base_limit > 0:
                        real_or_other_part = max(0, base_limit - funded_limit)
                        target_limit = max(target_limit, real_or_other_part + max(used, funded_limit - reducible))
                    if target_limit < data_limit:
                        fields["data_limit"] = target_limit
                        applied_gb = reducible / marzban_api.GB

                if penalty_days > 0 and funded_days > 0 and expire > now_ts:
                    # تعداد روزی که این سفارش با پول رفرال خریده شده؛ تمدید/روز واقعی محافظت می‌شود.
                    reducible_days = min(float(penalty_days), funded_days)
                    seconds = int(reducible_days * 86400)
                    if seconds > 0:
                        # کاهش expire هرگز بیشتر از سهم رفرالی همین سفارش نیست.
                        target_expire = max(now_ts, expire - seconds)
                        if target_expire < expire:
                            fields["expire"] = target_expire
                            applied_days = int(reducible_days)

                if not fields:
                    continue
                await marzban_api.modify_user(username, **fields)
                # Persist the remaining referral-funded entitlement so repeated leave/rejoin
                # cycles can never reduce the real-money portion of this order.
                new_funded_limit = funded_limit
                if "data_limit" in fields:
                    new_funded_limit = max(0, funded_limit - int(round(applied_gb * marzban_api.GB)))
                new_funded_days = funded_days
                if "expire" in fields:
                    new_funded_days = max(0.0, funded_days - float(applied_days))
                await database.execute(
                    "UPDATE orders SET referral_funded_data_limit=?, referral_funded_days=? WHERE id=?",
                    [new_funded_limit, new_funded_days, order["id"]],
                )
                ...
                service_error = True
                await database.log_action("error", referral["referrer_id"], f"خطا در جریمه سهم رفرالی سرویس #{rid}: {type(exc).__name__}: {exc}")

    # خطای پنل هرگز نباید به جریمه موفق تبدیل شود؛ worker بعدی دوباره امتحان می‌کند.
    if service_error and not service_changed and remaining_cash > 0:
        return False

    # اگر هنوز جریمه نقدی باقی است و سرویس رفرالی هم برای کسر امن وجود ندارد، بدهی ثبت می‌شود.
    debt = 0
    if remaining_cash > 0 and not service_changed:
        debt = await database.add_referral_penalty_debt(referral["referrer_id"], remaining_cash)
    await database.mark_referral_penalty_done(rid, cash_paid, applied_gb, applied_days, service_order_id)
    await database.log_action("referral_penalty", referral["referrer_id"], f"رفرال #{rid}; لفت {referral['referred_id']} از {channel}; نقدی={cash_paid}; سرویس={service_changed}; بدهی={debt}")
    try:
        referred_user = await database.get_user(referral["referred_id"])
        if referred_user:
            first = str(referred_user.get("first_name") or "").strip()
            last = str(referred_user.get("last_name") or "").strip()
            display_name = " ".join(x for x in (first, last) if x).strip() or "بدون نام"
            username_raw = str(referred_user.get("username") or "").strip()
            username_text = f"@{username_raw.lstrip('@')}" if username_raw else "ندارد"
        else:
            display_name = "کاربر حذف‌شده / نامشخص"
            username_text = "ندارد"

        penalty_lines = []
        if cash_paid:
            penalty_lines.append(f"💸 کسر از پاداش رفرال: <b>{cash_paid:,} تومان</b>")
        if service_changed:
            penalty_lines.append(f"📦 کسر از سهم رفرالی سرویس: <b>{applied_gb:g} گیگ</b> و <b>{applied_days} روز</b>")
        if debt:
            penalty_lines.append(f"🧾 بدهی جریمه رفرال: <b>{debt:,} تومان</b>")
        penalty_details = "\n".join(penalty_lines) if penalty_lines else "ℹ️ <b>موردی برای کسر ثبت نشد.</b>"
        ...
        text = template.format(
            ...
            user_id=html.escape(str(referral["referred_id"])),
            username=html.escape(username_text),
            channel=html.escape(str(channel)),
            penalty_details=penalty_details,
        )
        await context.bot.send_message(chat_id=referral["referrer_id"], text=text, parse_mode="HTML")
    except Exception:
        pass
    return True


_referral_penalty_scan_lock = asyncio.Lock()


async def scan_referral_penalties(context):
    """V11: detect real member->left transitions per referral/channel.
    A leave is penalized once per transition; rejoin resets the state so a later
    leave is a new event. Telegram errors/timeouts are fail-closed and never penalize.
    """
    if _referral_penalty_scan_lock.locked():
        return
    async with _referral_penalty_scan_lock:
        try:
            if await database.get_setting("referral_penalty_enabled", "0") != "1":
                return
            ...
            if not channels:
                return
            pending_events = await database.list_pending_referral_penalty_events(limit=100)
            for event in pending_events:
                if not await database.claim_referral_penalty_event(event["id"]):
                    continue
                referral = await database.get_referral_by_id(event["referral_id"])
                if referral:
                    referral["penalty_event_id"] = event["id"]
                    referral["left_at"] = event["left_at"]
                    referral["left_channel"] = event["channel"]
                    await _apply_referral_penalty(context, referral, event["channel"])
                    await database.mark_referral_penalty_pending_done(event["id"])
            candidates = await database.list_referral_penalty_candidates(limit=100)
            sem = asyncio.Semaphore(8)

            async def apply_claimed(referral, channel):
                event_id = referral.get("penalty_event_id")
                if not event_id:
                    return
                if not await database.claim_referral_penalty_event(event_id):
                    return
                ok = await _apply_referral_penalty(context, referral, channel)
                if ok:
                    await database.mark_referral_penalty_pending_done(event_id)

            async def one(referral):
                async with sem:
                    for c in channels:
                        is_member, _status = await check_membership_strict(context, c, referral["referred_id"])
                        if is_member is None:
                            # Unknown Telegram result: do not mutate state.
                            continue
                        if is_member:
                            await database.mark_referral_channel_rejoined(referral["id"], c)
                            continue
                        claimed = await database.claim_referral_leave(referral["id"], c)
                        if not claimed:
                            # Already out / event already exists.
                            continue
                        await apply_claimed(claimed, c)

            await asyncio.gather(*(one(r) for r in candidates), return_exceptions=True)
        except Exception:
            logger.exception("خطا در پایش جریمه رفرال V11")


async def start(update, context):
    user = update.effective_user
    # این دو کار مستقل‌اند؛ موازی‌سازی‌شان زمان پاسخ /start را کم می‌کند بدون تغییر در منطق داده.
    _, startup_settings = await asyncio.gather(
        database.upsert_user(user.id, user.username or ""),
        database.get_settings_many(["referral_enabled", "bot_enabled"], {"referral_enabled": "0", "bot_enabled": "1"}),
    )
    referral_enabled = startup_settings.get("referral_enabled") or "0"
    # V11 ضدسوءاستفاده: پنجره attribution فقط در اولین /start وجود دارد.
    # /start خالی، payload قدیمی/نامعتبر، یا هر شروع دیگری بعد از اولین شروع،
    # دیگر اجازه ساخت رفرال را نمی‌دهد. این تصمیم اتمیک است و در برابر /start همزمان مقاوم است.
    referral_arg = str(context.args[0]).strip() if context.args else ""
    referrer_id = database.resolve_referral_code(referral_arg) if (referral_arg and referral_enabled == "1") else None
    first_start = await database.claim_first_referral_start(user.id)
    if first_start and referrer_id is not None:
        if referrer_id == user.id:
            await update.effective_message.reply_text(
                "❌ <b>این لینک دعوت برای خودت ساخته شده!</b>\n"
                "هر کاربر فقط می‌تواند دیگران را دعوت کند؛ با لینک خودت نمی‌توانی برای خودت دعوت ثبت کنی. 🔒",
                parse_mode="HTML",
            )
            return
        await database.set_pending_referrer(user.id, referrer_id)
    # بررسی موازی همه وضعیت‌های کاربر برای سریع‌ترین پاسخ ممکن به /start
    is_admin, is_blocked, rules_accepted, bot_enabled_val = await asyncio.gather(
        database.is_admin(user.id),
        database.is_user_blocked(user.id),
        database.has_accepted_rules(user.id),
        database.get_setting("bot_enabled", "1"),
    )
    if not is_admin:
        if is_blocked:
            try:
...
async def check_join_callback(update, context):
    ...
    if not missing:
        await query.answer("✅ عضویت همه کانال‌ها تأیید شد؛ دسترسی باز شد.", show_alert=True)
        try:
            await query.message.delete()
        except Exception:
            pass
    ...
    count = len(missing)
    names = "، ".join(_forcejoin_channel_label(c) for c in missing)
    if count == 1:
        msg = f"❌ هنوز عضو این کانال نیستی: {names}\nاول عضو شو و دوباره بررسی کن."
    else:
        msg = f"❌ هنوز عضویت کامل نشده؛ {count} کانال باقی مانده:\n{names}\n\nبعد از عضویت در همه دوباره بررسی کن."
    # show_alert باعث می‌شود نتیجه همان لحظه بالای چت به کاربر اعلام شود.
    await query.answer(msg, show_alert=True)
    try:
        await query.message.edit_text(
            "🔒 <b>عضویت هنوز کامل نشده است.</b>\n\n"
            f"کانال‌های باقی‌مانده: <b>{count}</b>\n{chr(10).join('• ' + _forcejoin_channel_label(c) for c in missing)}\n\n"
            "پس از عضویت در همه، روی «✅ بررسی عضویت من» بزن.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                [ibtn(f"📢 عضویت در {_forcejoin_channel_label(c)}", url=_forcejoin_channel_url(c), style="primary")]
                for c in missing
            ] + [[ibtn("✅ بررسی عضویت من", callback_data="check_join", style="success")]]),
        )
    except Exception:
        pass
...
async def main_menu_router(update, context):
    ...
    # مسیر منوی اصلی نباید به‌خاطر یک وابستگی خارجی برای همیشه بی‌پاسخ بماند.
    # DB/Telegram checkها سقف زمانی دارند و در outage موقت اجازه می‌دهیم handler واقعی ادامه پیدا کند؛
    # اگر پاسخ قطعی blocked یا bot-off باشد، ensure_bot_active خودش جلوی مسیر را می‌گیرد.
    try:
        active_ok = await asyncio.wait_for(ensure_bot_active(update, context, user_id), timeout=2.0)
    except asyncio.TimeoutError:
        logger.warning("main menu access timeout user=%s; continuing", user_id)
        active_ok = True
    except Exception:
        logger.exception("main menu access check failed user=%s; continuing", user_id)
        active_ok = True
    if not active_ok:
        return

    try:
        is_admin_user = await asyncio.wait_for(database.is_admin(user_id), timeout=1.5)
    except Exception:
        is_admin_user = False
    if not is_admin_user:
        try:
            missing = await asyncio.wait_for(get_missing_channels(context, user_id), timeout=2.5)
        except asyncio.TimeoutError:
            logger.warning("main menu forcejoin timeout user=%s; continuing", user_id)
            missing = []
        except Exception:
            logger.exception("main menu forcejoin check failed user=%s; continuing", user_id)
            missing = []
        if missing:
    ...
    try:
        await asyncio.wait_for(handler_fn(update, context), timeout=20.0)
    except asyncio.TimeoutError:
        logger.error("main menu handler timeout user=%s button=%r", user_id, text)
        try:
            await update.effective_message.reply_text("⏳ درخواست بیش از حد طول کشید. لطفاً دوباره همان گزینه را بزنید.")
        except Exception:
            pass
...
DEFAULT_PACKAGE_LABEL_TEMPLATE = "🚀 {gb} گیگ | {duration} کاربر ∞ — {price} تومان"

PACKAGE_LABEL_TEMPLATES = [
    "🚀 {gb} گیگ | {duration} کاربر ∞ — {price} تومان",
    "🎉{gb} گیگ | {duration} کاربر (علامت بی نهایت) - {price} تومان🎉",
    "⚡️ {gb} گیگابایت | {duration} | فقط {price} تومان",
    "💎 پلن {gb} گیگی | {duration} | {price} تومان 💎",
    "🔥 {gb}GB — {duration} — {price} تومان 🔥",
    "🌟 {name} | {gb} گیگ | {duration} | {price} تومان",
    "📦 {gb} گیگ / {duration} ⟜ فقط {price} تومان",
    "✅ {gb} گیگ پرسرعت | {duration} | ∞کاربر | {price} تومان",
    "🎁 {gb} گیگ هدیه‌وار! {duration} فقط {price} تومان 🎁",
    "🛡 {gb} گیگ امن | مدت {duration} | {price} تومان 🛡",
    "《{gb} گیگ》 {duration} — {price} تومان",
]


def _package_label_context(p):
...
def render_package_label_strict(template, p):
    """رندر یک قالب بدون fallback؛ اگر قالب نامعتبر باشد (مثلا متغیر ناشناس) خطا را پرتاب می‌کند؛
    فقط برای اعتبارسنجی قالب دلخواه ادمین استفاده می‌شود، نه برای نمایش واقعی به کاربر."""
    return template.format(**_package_label_context(p))


def validate_package_label_template(template):
    """قالب دلخواه ادمین را با یک پلن نمونه تست می‌کند؛ اگر معتبر بود پیش‌نمایش را برمی‌گرداند،
    ورنه None یعنی قالب نامعتبر است."""
    sample = {"name": "نمونه", "gb": 150, "days": 60, "price": 444000}
    try:
        return render_package_label_strict(template, sample)
    except Exception:
        return None


def render_package_label(template, p):
    """مانند render_package_label_strict اما اگر قالب خراب شود (مثلا یک قالب دلخواه قدیمی که دیگر معتبر نیست)،
    هرگز برای مشتری دکمه خطا نشان نمی‌دهد و به قالب پیش‌فرض برمی‌گردد."""
    try:
        return render_package_label_strict(template, p)
    except Exception:
        return render_package_label_strict(DEFAULT_PACKAGE_LABEL_TEMPLATE, p)


def pkg_template_setting_key(category_id=None):
    """کلید ذخیره‌سازی قالب نمایش پلن مخصوص هر دسته (مثلا هفتگی/ماهانه)؛ category_id=None یعنی
    دسته «سایر پلن‌ها» (بدون دسته). هر دسته قالب خودش را جدا از بقیه دسته‌ها دارد."""
    return f"package_label_template:cat:{category_id if category_id is not None else 'none'}"


async def get_package_label_template(category_id=None):
    return await database.get_setting(pkg_template_setting_key(category_id), DEFAULT_PACKAGE_LABEL_TEMPLATE)


def _format_package_label(p, template=None):
    return render_package_label(template or DEFAULT_PACKAGE_LABEL_TEMPLATE, p)


DEFAULT_BUY_PROMPT_TEXT = "🛍️ خب، یکی از گزینه‌های زیر رو انتخاب کنید:"

BUY_PROMPT_TEXT_PRESETS = [
    "🛍️ خب، یکی از گزینه‌های زیر رو انتخاب کنید:",
    "🎯 یکی از پلن‌های زیر رو انتخاب کن و شروع کن!",
    "✨ کدوم پلن مورد علاقه‌ته؟ یکیو انتخاب کن:",
    "🛒 وقت خریده! از بین گزینه‌های زیر انتخاب کن:",
    "🔥 پلن‌های پرطرفدار پایین‌تره، یکیو انتخاب کن:",
    "📲 برای شروع، یکی از گزینه‌های زیر رو بزن:",
    "💡 یکی از پلن‌های زیر رو برای ادامه انتخاب کنید:",
    "🚀 آماده‌ای؟ یکی از گزینه‌های زیر رو انتخاب کن:",
    "🎁 بهترین پلن برای تو کدومه؟ انتخاب کن:",
    "⭐️ از بین گزینه‌های زیر، یکی رو انتخاب کن:",
    "👇 پلن دلخواهت رو از پایین انتخاب کن:",
]
...
async def _buy_categories_view():
    ...
    if len(keyboard) <= 1:
        ...
        prompt_text = await database.get_setting("buy_prompt_text", DEFAULT_BUY_PROMPT_TEXT)
        text = f"<b>{prompt_text}</b>"
...
async def buy_category_callback(update, context):
    ...
    label_template = await get_package_label_template(category_id)
    keyboard = [[ibtn(_format_package_label(p, label_template), callback_data=f"pkg:{p['id']}", style="primary")] for p in packages]
    keyboard.append([ibtn("🔙 بازگشت", callback_data="buy_categories_back", style="danger")])
    if packages:
        text = f"🛍️ <b>{category_name}</b>\n━━━━━━━━━━━━━━━\n<b>یکی از پلن‌های زیر را انتخاب کنید:</b>"
    else:
...
def _rial(amount_toman):
    return int(amount_toman) * 10


def _copy_button(label, text):
    return InlineKeyboardButton(label, api_kwargs={"copy_text": {"text": str(text)}})


async def _effective_pay_amount(context, base_price):
    if context.user_data.get("requested_price"):
        return int(context.user_data["requested_price"])
    mode = await database.get_setting("payment_amount_mode", "fixed")
    amount = int(base_price or 0)
    if mode == "randomized" and amount > 0:
        try:
            mn = int(await database.get_setting("payment_random_min", "100") or 100)
            mx = int(await database.get_setting("payment_random_max", "1500") or 1500)
            if mx < mn:
                mn, mx = mx, mn
            amount += random.randint(max(0, mn), max(0, mx))
        except Exception:
            amount += random.randint(100, 1500)
    context.user_data["requested_price"] = amount
    return amount


async def _guide_keyboard_rows():
    enabled = await database.get_setting("post_approval_guide_enabled", "0") == "1"
    if not enabled:
        return []
    url = (await database.get_setting("post_approval_guide_url", "") or "").strip()
    if url:
        return [[ibtn("📚 آموزش اتصال", url=url, style="primary")]]
    return [[ibtn("📚 آموزش اتصال", callback_data="show_connection_guide", style="primary")]]


async def show_connection_guide_callback(update, context):
    query = update.callback_query
    try:
        await query.answer()
    except Exception:
        pass
    text = await database.get_setting("post_approval_guide_text", "📚 آموزش اتصال هنوز تنظیم نشده است.")
    await query.message.reply_text(text, parse_mode="HTML")


async def _show_card_payment(update_or_query, context, final_price):
    requested_price = await _effective_pay_amount(context, final_price)
    context.user_data["final_price"] = final_price
    card_number = await database.get_setting("card_number", "-")
    card_holder = await database.get_setting("card_holder", "-")
    mode = await database.get_setting("payment_amount_mode", "fixed")
    rial_amount = _rial(requested_price)
    if mode == "randomized" and requested_price != final_price:
        text = (
            "💳 <b>پرداخت کارت به کارت — مبلغ اختصاصی شما</b>\n"
            "━━━━━━━━━━━\n"
            f"🧾 مبلغ پایه سفارش: <b>{final_price:,} تومان</b>\n"
            f"✅ <b>مبلغ دقیق قابل پرداخت:</b> <code>{requested_price:,}</code> تومان\n"
            f"💵 معادل ریالی برای کپی: <code>{rial_amount:,}</code> ریال\n\n"
            f"💳 <b>شماره کارت:</b> <code>{card_number}</code>\n👤 <b>به نام:</b> {card_holder}\n\n"
            "⚠️ <b>مبلغ را بدون رُند کردن و دقیقاً مطابق عدد بالا واریز کنید.</b>\n"
            "این مبلغ اختصاصی برای تشخیص سریع‌تر رسید شما ساخته شده؛ اگر مبلغ را تغییر دهید، تایید پرداخت ممکن است با تأخیر یا مشکل انجام شود.\n\n"
        ...
        text = (
            ...
            "━━━━━━━━━━━\n"
            f"💵 <b>مبلغ قابل پرداخت:</b> <code>{requested_price:,}</code> تومان\n"
            f"💵 معادل ریالی برای کپی: <code>{rial_amount:,}</code> ریال\n\n"
            f"💳 <b>شماره کارت:</b> <code>{card_number}</code>\n👤 <b>به نام:</b> {card_holder}\n\n"
            "📸 <b>لطفا بعد از واریز، عکس رسید پرداخت را همینجا ارسال کنید.</b>"
        )
    keyboard = InlineKeyboardMarkup([
        [_copy_button("📋 کپی شماره کارت", card_number)],
        [_copy_button("💵 کپی مبلغ به ریال", str(rial_amount))],
...
async def flow_cancel_callback(update, context):
    ...
    context.user_data.pop("requested_price", None)
...
async def receive_receipt(update, context):
    ...
    # اطمینان از وجود ردیف کاربر در جدول users پیش از ثبت سفارش، ورنه اگر این کاربر به هر دلیلی قبلاً در جدول ثبت نشده باشد
    # (مثلاً بدون /start)، بعداً جستجوی ادمین در بخش کاربران با آیدی عددی/یوزرنیم نتیجه‌ای ندهد.
    await database.upsert_user(update.effective_user.id, update.effective_user.username or "")
    order_id = await database.create_order(
        ...
        custom_gb=custom_gb, custom_days=custom_days, requested_price=context.user_data.get("requested_price", final_price),
    )
    await database.update_order(order_id, payment_source="card", referral_funds_used=0)
    if not package:
...
async def _post_trust_channel(context, buyer_user, service_username, package, final_price, expire_ts):
    try:
        ...
        buyer_id_masked = _mask_user_id(buyer_id)

        # انتخاب قالب از DB
        import admin_core as _ac
        tpl_idx = int(await database.get_setting("trust_channel_template_idx", "0"))
        tpl_idx = max(0, min(tpl_idx, len(_ac.TRUST_CHANNEL_TEMPLATES) - 1))
        template = _ac.TRUST_CHANNEL_TEMPLATES[tpl_idx]["template"]
        text = template.format(
            buyer_name=html.escape(str(buyer_name)),
            buyer_id_masked=buyer_id_masked,
            service_username=html.escape(str(service_username)),
            gb=package["gb"], days=package["days"],
            price=f"{final_price:,}",
            expire_date=expire_date,
            now_str=now_str,
        )

        # دکمه خرید زیر پیام (اختیاری)
        ...
        if buy_btn_enabled and buy_btn_url:
            ...
            reply_markup = IKM([[InlineKeyboardButton(buy_btn_label, url=buy_btn_url)]])

        await context.bot.send_message(chat_id=channel, text=text, parse_mode="HTML", reply_markup=reply_markup)
    except Exception as error:
        try:
...
async def _fulfill_order(order, context):
    ...
    selected_groups_raw = await database.get_setting("pasarguard_selected_group_ids", "all")
    selected_groups = None if selected_groups_raw == "all" else __import__('json').loads(selected_groups_raw)
    user_data = await marzban_api.create_user(package["gb"], package["days"], username=desired_username, note=note, group_ids=selected_groups)
    ...
    if not link or not isinstance(link, str) or not link.startswith("http"):
        await database.log_action("error", buyer_id, f"QR safety check failed for order #{order['id']}: invalid subscription link")
        raise Exception("پنل لینک اشتراک معتبر برنگرداند؛ برای جلوگیری از ارسال QR اشتباه، ساخت سرویس متوقف شد.")
    if service_username and service_username != order.get("desired_username", service_username):
        await database.log_action("error", buyer_id, f"QR safety notice order #{order['id']}: panel returned username {service_username}")
    if not service_username or service_username not in str(user_data):
        await database.log_action("error", buyer_id, f"QR/link safety check warning for order #{order['id']} service={service_username}")
    await database.update_order(
        order["id"], status="approved", marzban_username=service_username, subscription_link=link,
        referral_base_data_limit=int(user_data.get("data_limit") or 0),
        referral_base_expire=int(marzban_api.parse_expire_timestamp(user_data.get("expire")) if hasattr(marzban_api, "parse_expire_timestamp") else int(user_data.get("expire") or 0)),
    )
    # در صورت فعال بودن شرط رفرال، خرید واجدشرایط این کاربر پاداش معرف او را آزاد می‌کند.
    referral_unlocked = await database.qualify_referral_for_order(order["id"])
    if referral_unlocked:
        try:
            await context.bot.send_message(
                chat_id=(await database.get_referral_by_referred(order["user_id"]))["referrer_id"],
                text="🎉 <b>پاداش رفرال شما آزاد شد!</b>\n"
                     "دوست دعوت‌شده‌تان خرید واجدشرایط انجام داد و پاداش این دعوت اکنون به کیف پول شما اضافه شد. 💰",
                parse_mode="HTML",
            )
        except Exception:
            pass
    ...
    guide_rows = await _guide_keyboard_rows()
    guide_markup = InlineKeyboardMarkup(guide_rows) if guide_rows else None
    await context.bot.send_photo(chat_id=order["user_id"], photo=qr_bytes, caption=caption, parse_mode="HTML", reply_markup=guide_markup)
    try:
...
async def order_approve_callback(update, context):
    ...
    # --- رفع باگ Race Condition: اگر دو ادمین همزمان تایید بزنند ---
    # claim_order_for_approval اتمیک است: فقط یکی موفق می‌شود status را به 'processing' تغییر دهد.
    # ادمین دوم "قبلاً بررسی شده" می‌بیند و سرویس دوتایی ساخته نمی‌شود.
    claimed = await database.claim_order_for_approval(order_id)
    if not claimed:
    ...
    try:
        ...
        # در صورت خطا، status رو به rejected برمی‌گردانیم تا ادمین بتواند دوباره تلاش کند
        await database.update_order(order_id, status="pending")
...
async def trial_run(update, context):
    ...
    if await database.get_setting("trial_enabled", "0") != "1":
        ...
        return
    # اتمیک claim: اگر کاربر دوبار سریع دکمه تست بزند، فقط یکی موفق می‌شود
    claimed = await database.claim_trial_slot(user_id)
    if not claimed:
    ...
    try:
        ...
        gb = 1.0
    try:
        trial_amount = float(await database.get_setting("trial_days", "1"))
        trial_unit = await database.get_setting("trial_unit", "days")
        days = trial_amount / 24 if trial_unit == "hours" else trial_amount
    except (TypeError, ValueError):
        days = 1
    try:
        try:
        ...
        selected_groups_raw = await database.get_setting("pasarguard_selected_group_ids", "all")
        selected_groups = None if selected_groups_raw == "all" else json.loads(selected_groups_raw)
        user_data = await marzban_api.create_user(gb, days, note=note, group_ids=selected_groups)
        link = marzban_api.extract_subscription_link(user_data)
        service_username = user_data.get("username", "")
        if not service_username or service_username not in str(user_data):
            await database.log_action("error", user_id, f"QR/link safety check warning for trial service={service_username}")
        # مطمئن شدن از وجود ردیف کاربر در جدول users (تا جستجوی ادمین بر اساس آیدی عددی همیشه کار کند)
        await database.upsert_user(user_id, update.effective_user.username or "")
        ...
        caption = (
            ...
            f"👤 <b>نام کاربری سرویس:</b> <code>{service_username}</code>\n"
            "📡 <b>نوع:</b> 🎁 تست رایگان\n"
            f"⏳ <b>مدت زمان:</b> {trial_amount:g} {'ساعت' if trial_unit == 'hours' else 'روز'}\n"
        ...
        # اگر ساخت سرویس شکست خورد، claim رو آزاد کن تا کاربر بتواند دوباره امتحان کند
        try:
            await database.execute(
                "UPDATE users SET used_trial = 0, trial_generation = NULL WHERE user_id = ? AND used_trial = 1",
                [user_id],
            )
        except Exception:
            pass
...
async def connection_guide_show(update, context):
    """نمایش راهنمای اتصال برای کاربران - محتوا از تنظیمات مدیریت می‌آید"""
    title = await database.get_setting("connection_guide_title", "📲 آموزش اتصال")
    text = await database.get_setting("connection_guide_text", "📲 <b>آموزش اتصال</b>\n\nراهنمای اتصال هنوز توسط مدیریت تنظیم نشده است.")
    btn_label_1 = await database.get_setting("connection_guide_btn1_label", "")
    btn_url_1 = await database.get_setting("connection_guide_btn1_url", "")
    btn_label_2 = await database.get_setting("connection_guide_btn2_label", "")
    btn_url_2 = await database.get_setting("connection_guide_btn2_url", "")
    btn_label_3 = await database.get_setting("connection_guide_btn3_label", "")
    btn_url_3 = await database.get_setting("connection_guide_btn3_url", "")
    keyboard_rows = []
    for label, url in [(btn_label_1, btn_url_1), (btn_label_2, btn_url_2), (btn_label_3, btn_url_3)]:
        if label and url:
    ...
    await update.message.reply_text(text, parse_mode="HTML", reply_markup=keyboard, disable_web_page_preview=False)
...
async def _build_account_detail(order):
    ...
    order_id = order["id"]
    try:
        info = await asyncio.wait_for(marzban_api.get_user(username), timeout=18)
    except Exception as e:
        error_str = str(e)
    ...
    if limit:
        ...
        if used_percent >= 90:
            ...
            usage_note = "\n\u2139\ufe0f بیشتر از نیمی از حجمت مصرف شده، حواست را داشته باش."
        else:
            usage_note = ""
    else:
    ...
    online_at = info.get("online_at") or info.get("last_online") or info.get("last_connected_at")
    last_connection = "اطلاعاتی از پنل دریافت نشد" if not online_at else str(online_at).replace("T", " ").split(".")[0]
    sub_updated_at = info.get("sub_updated_at") or info.get("subscription_updated_at") or info.get("updated_at")
    sub_updated_str = "اطلاعاتی از پنل دریافت نشد" if not sub_updated_at else str(sub_updated_at).replace("T", " ").split(".")[0]
    user_agent = info.get("sub_last_user_agent") or info.get("last_user_agent") or "اطلاعاتی از پنل دریافت نشد"

    product_name = "🎁 تست رایگان" if is_trial else package.get("name", "سرویس")
    price_line = "" if is_trial else f"💰 <b>مبلغ پرداخت‌شده:</b> {_to_persian_digits(order.get('price', 0))} تومان\n"
    time_section = f"⏳ <b>زمان باقی‌مانده:</b>\n{time_bar}\n" if time_bar else ""

    # آیکون وضعیت پیشرفته
    if is_expired:
        status_icon, status_label = "💀", "منقضی شده"
    elif not is_active:
        status_icon, status_label = "⏸", "خاموش (توسط شما)"
    else:
        status_icon, status_label = "✅", "فعال و آنلاین"

    # استفاده از قالب انتخاب‌شده توسط ادمین
    try:
        import admin_core as _ac
        tpl_idx = int(await database.get_setting("service_template_idx", "0"))
        tpl_idx = max(0, min(tpl_idx, len(_ac.SERVICE_TEMPLATES) - 1))
        template = _ac.SERVICE_TEMPLATES[tpl_idx]["template"]
        text = template.format(
            status_icon=status_icon, status_label=status_label,
            username=username, product_name=product_name,
            price_line=price_line, order_id=order_id,
            usage_bar=usage_bar, used_str=used_str,
            limit_str=limit_str, remaining_str=remaining_str,
            usage_note=usage_note, time_section=time_section,
            expire_str=expire_str, countdown=countdown,
        ...
        text = (
            ...
            f"📦 <b>محصول:</b> {product_name}\n"
            f"{price_line}"
            f"🆔 <b>سفارش:</b> #{order_id}\n"
            f"━━━━━━━━━━━━━━━\n"
            f"📊 <b>ترافیک:</b>\n{usage_bar}\n"
            f"📥 {used_str}  •  📦 {limit_str}\n"
            f"🛟 باقی: <b>{remaining_str}</b>{usage_note}\n"
            f"━━━━━━━━━━━━━━━\n"
            f"{time_section}"
            f"📅 {expire_str}{countdown}{expiry_note}"
        )
    action_rows = [
        [ibtn("♻️ بروزرسانی", callback_data=f"acc_refresh:{order_id}"), ibtn("🔗 لینک اشتراک", callback_data=f"acc_link:{order_id}")],
        [ibtn("📗 دریافت کانفیگ", callback_data=f"acc_config:{order_id}"), ibtn("⚙️ تعویض لینک", callback_data=f"acc_revoke:{order_id}")],
    ]
    if not is_trial:
        action_rows.insert(0, [ibtn("💊 تمدید سرویس", callback_data=f"pkg:{package.get('id', 0)}", style="success")])
    guide_rows = await _guide_keyboard_rows()
    if guide_rows:
        action_rows.extend(guide_rows)
    # فقط اگر سرویس منقضی نشده باشد، دکمه خاموش/روشن کردن نمایش داده می‌شود
    if not is_expired:
...
async def account_config_callback(update, context):
    ...
    try:
        ...
        if not links:
            sub_link = order.get("subscription_link") or marzban_api.extract_subscription_link(info)
            try:
                links = await asyncio.wait_for(marzban_api.get_subscription_configs(sub_link), timeout=18)
            except Exception as sub_error:
                await database.log_action("error", query.from_user.id, f"خطا در استخراج کانفیگ از ساب {username}: {sub_error}")
                links = []
        if links:
            await query.message.reply_text("📗 <b>کانفیگ‌های اختصاصی همین سرویس شما آماده شد.</b>\nبرای کپی راحت، هر کانفیگ در یک پیام جدا ارسال می‌شود 👇", parse_mode="HTML")
            for link in links:
                await query.message.reply_text(f"<code>{link}</code>", parse_mode="HTML")
        else:
            await query.message.reply_text("⚠️ پنل برای این سرویس کانفیگ جداگانه برنگرداند. لطفاً از دکمه «لینک اشتراک» استفاده کنید یا با پشتیبانی پیام بدهید.", parse_mode="HTML")
    except Exception as e:
...
async def referral_show(update, context):
    user_id = update.effective_user.id
    if await database.get_setting("referral_enabled", "0") != "1":
        await update.message.reply_text("ℹ️ سیستم دعوت دوستان در حال حاضر فعال نیست.")
        return
    async def _bot_username():
        try:
            ...
            return ""
    code, bot_username, summary, reward_raw, condition_raw, min_gb = await asyncio.gather(
        database.get_or_create_referral_code(user_id),
        _bot_username(),
        database.referral_summary(user_id),
        database.get_setting("referral_reward", "5000"),
        database.get_setting("referral_condition_enabled", "1"),
        database.get_setting("referral_min_gb", "10"),
    )
    if not bot_username:
        await update.message.reply_text("⚠️ لینک دعوت در حال حاضر قابل ساخت نیست. لطفاً بعداً دوباره تلاش کنید.")
        return
    link = f"https://t.me/{bot_username}?start={code}"
    reward = int(float(reward_raw or 0))
    condition = condition_raw == "1"
    try:
        min_gb_text = str(int(float(min_gb))) if float(min_gb).is_integer() else str(min_gb)
    except Exception:
        min_gb_text = str(min_gb)
    if condition:
        condition_text = (
            f"🔐 <b>قانون آزادسازی:</b> مجموع پاداش‌های دعوت فعلاً در انتظار می‌ماند. "
            f"وقتی <b>تمام دوستانی که با لینک شما دعوت موفق شده‌اند</b> حداقل یک پلن "
            f"<b>{min_gb_text} گیگ</b> یا بیشتر خریداری کنند، مجموع پاداش‌های در انتظار یکجا آزاد و قابل استفاده می‌شود."
        )
    else:
        condition_text = "⚡️ <b>وضعیت:</b> شرط خرید خاموش است؛ هر دعوت موفق پس از تکمیل عضویت، پاداش را فوراً قابل استفاده می‌کند."
    template = await database.get_setting(
        "referral_page_text",
    ...
    try:
        text = template.format(
            reward=reward, link=html.escape(link), code=html.escape(code),
            total=summary["total"], successful=summary["successful"],
            unlocked=summary["unlocked"], locked=summary["locked"],
            condition_text=condition_text,
        )
    except Exception:
        text = (
            f"👥 <b>دعوت دوستان و کسب درآمد</b> 💸\n━━━━━━━━━━━━━━\n"
            f"به‌ازای هر دعوت موفق <b>{reward:,} تومان</b> پاداش بگیر! 🎁\n\n"
            f"🔗 <b>لینک اختصاصی:</b>\n<code>{html.escape(link)}</code>\n\n"
            f"🔑 <b>کد:</b> <code>{html.escape(code)}</code>\n"
            f"👤 تعداد دعوت: <b>{summary['total']}</b>\n"
            f"✅ دعوت موفق: <b>{summary['qualified']}</b>\n"
            f"🔓 آزاد شده: <b>{summary['unlocked']:,} تومان</b>\n"
            f"🔒 در انتظار: <b>{summary['locked']:,} تومان</b>\n\n{condition_text}"
        )
    await update.message.reply_text(text, parse_mode="HTML")
...
async def wallet_topup_approve_callback(update, context):
    ...
    # atomic claim: فقط اولین ادمینی که تایید می‌زند موفق می‌شود (race condition fix)
    claimed = await database.claim_wallet_topup(tx_id)
    if not claimed:
        await query.answer("این درخواست همین الان توسط ادمین دیگری بررسی شد.", show_alert=True)
        return
    # شارژ واقعی همیشه به موجودی عادی اضافه می‌شود؛ علاوه بر آن، به همان اندازه از
    # پاداش‌های قفل‌شده رفرال آزاد می‌کنیم (تا سقف مبلغ قفل‌شده). این دو منبع در UI یکی هستند.
    new_balance = await database.adjust_wallet_balance(tx["user_id"], tx["amount"])
    debt_settled = 0
    try:
        debt_settled = await database.settle_referral_penalty_debt_from_topup(tx["user_id"], tx["amount"])
        if debt_settled:
            new_balance = await database.get_wallet_balance(tx["user_id"])
    except Exception:
        logger.exception("خطا در تسویه بدهی رفرال از شارژ واقعی tx=%s", tx_id)
    # فقط بخش باقی‌مانده‌ی شارژ واقعی که صرف تسویه بدهی نشده، می‌تواند
    # همان مقدار از پاداش قفل‌شده را آزاد کند؛ یک تومان دوبار مصرف نمی‌شود.
    release_base = max(0, int(tx["amount"]) - int(debt_settled or 0))
    released_by_topup = 0
    try:
        released_by_topup = await database.release_referral_by_real_topup(tx["user_id"], release_base)
        if released_by_topup:
            new_balance = await database.get_wallet_balance(tx["user_id"])
    except Exception:
        logger.exception("خطا در آزادسازی رفرال متناسب با شارژ واقعی برای tx=%s", tx_id)
    ...
    try:
        await context.bot.send_message(
            ...
            text=(f"✅ شارژ کیف پول شما به مبلغ {tx['amount']:,} تومان تایید شد. 🎉\n"
                  ...
                  + (f"\n🧾 {debt_settled:,} تومان از بدهی جریمه رفرال از همین شارژ تسویه شد." if debt_settled else "")
                  + (f"\n🔓 {released_by_topup:,} تومان از پاداش‌های دعوت هم به اندازه شارژ واقعی آزاد شد." if released_by_topup else "")),
        )
    except Exception:
        pass
...
async def wallet_topup_reject_callback(update, context):
    ...
    if not tx or tx["status"] != "pending":
        await query.answer("این درخواست قبلاً بررسی شده.", show_alert=True)
        return
    # atomic claim: جلوگیری از race condition
    claimed = await database.claim_wallet_topup_reject(tx_id)
    if not claimed:
        await query.answer("این درخواست همین الان توسط ادمین دیگری بررسی شد.", show_alert=True)
        return
    await query.answer("رد شد.")
...
async def _process_wallet_payment(update, context):
    ...
    if balance < final_price:
        if query:
            await query.answer()
        locked_referral = 0
        if await database.get_setting("referral_condition_enabled", "1") == "1":
            try:
                locked_referral = (await database.referral_summary(user_id))["locked"]
            except Exception:
                locked_referral = 0
        if locked_referral > 0:
            min_gb = await database.get_setting("referral_min_gb", "10")
            await _reply(
                f"🔐 <b>بخشی از پاداش‌های دعوت شما هنوز در انتظار آزادسازی است.</b>\n"
                f"💰 موجودی قابل استفاده: <b>{balance:,} تومان</b>\n"
                f"🔒 مبلغ در انتظار: <b>{locked_referral:,} تومان</b>\n\n"
                f"برای آزاد شدن مجموع پاداش‌های در انتظار، <b>تمام دوستانی که دعوت موفق شده‌اند</b> باید حداقل یک پلن "
                f"<b>{html.escape(str(min_gb))} گیگ</b> یا بیشتر خریداری کنند. پس از تکمیل شرط، مجموع مبلغ در انتظار به موجودی قابل استفاده منتقل می‌شود.\n\n"
                f"🧾 مبلغ موردنیاز این خرید: <b>{final_price:,} تومان</b>",
                parse_mode="HTML",
            )
        else:
            await _reply(
        ...
        return ConversationHandler.END
    package_gb = custom_gb if custom_gb is not None else (package.get("gb") if package else 0)
    referral_spend_enabled = await database.get_setting("referral_spend_enabled", "0") == "1"
    if referral_spend_enabled:
        try:
            spend_min_gb = float(await database.get_setting("referral_spend_min_gb", "10") or 10)
        except (TypeError, ValueError):
            spend_min_gb = 10.0
        try:
            referral_internal = await database.get_internal_referral_balance(user_id)
        except Exception:
            referral_internal = 0
        regular_internal = max(0, balance - referral_internal)
        referral_needed = max(0, final_price - regular_internal)
        if referral_needed > 0 and float(package_gb or 0) < spend_min_gb:
            await _reply(
                f"🔒 <b>فعلاً امکان خرید این پلن با موجودی شما نیست.</b>\n\n"
                f"برای مصرف پاداش دعوت در خرید، حداقل پلن مجاز <b>{spend_min_gb:g} گیگ</b> است.\n"
                f"📦 حجم پلن انتخابی: <b>{float(package_gb or 0):g} گیگ</b>\n"
                f"💰 مبلغ خرید: <b>{final_price:,} تومان</b>\n\n"
                "دو راه دارید: \n"
                f"1️⃣ به دعوت دوستان ادامه بدهید تا پاداش طبق شرط آزاد شود.\n"
                "2️⃣ کیف پول را با شارژ واقعی افزایش دهید؛ به ازای هر مقدار شارژ واقعی، همان مقدار از پاداش قفل‌شده آزاد می‌شود.\n\n"
                "ℹ️ در خریدهای مجاز، ابتدا پاداش آزادشده مصرف می‌شود و سپس موجودی عادی.",
                parse_mode="HTML",
            )
            return ConversationHandler.END
    if query:
    ...
    # مطمئن شدن از وجود ردیف کاربر در جدول users (تا جستجوی ادمین بر اساس آیدی عددی همیشه کار کند)
    _wallet_user = query.from_user if query else update.effective_user
    await database.upsert_user(user_id, (_wallet_user.username if _wallet_user else None) or "")
    order_id = await database.create_order(
        user_id, package_id, None, price=final_price, discount_code=discount_code,
        order_type="purchase", custom_gb=custom_gb, custom_days=custom_days, desired_username=desired_username, requested_price=final_price,
    )
    spent_ok, referral_used = await database.try_spend_wallet_for_purchase(user_id, final_price, package_gb)
    if not spent_ok:
        await database.update_order(order_id, status="rejected")
        if referral_used == -1:
            await _reply("🔒 <b>برای استفاده از موجودی دعوت دوستان، این پلن حداقل حجم مجاز را ندارد.</b>", parse_mode="HTML")
        else:
            await _reply("⚠️ <b>موجودی کیف پول دیگر کافی نیست. لطفاً دوباره تلاش کنید.</b>", parse_mode="HTML")
        return ConversationHandler.END
    await database.add_wallet_transaction(user_id, -final_price, "purchase", related_order_id=order_id)
    referral_used = max(0, int(referral_used or 0))
    referral_share_gb = 0.0
    referral_share_days = 0.0
    if referral_used > 0 and final_price > 0:
        referral_share = min(1.0, referral_used / float(final_price))
        referral_share_gb = max(0.0, float(package_gb or 0) * referral_share)
        if custom_days is not None:
            referral_share_days = max(0.0, float(custom_days or 0) * referral_share)
        elif package:
            referral_share_days = max(0.0, float(package.get("days") or 0) * referral_share)
    await database.update_order(
        order_id, referral_funds_used=referral_used,
        referral_funded_data_limit=int(referral_share_gb * marzban_api.GB),
        referral_funded_days=referral_share_days,
        payment_source=("wallet_referral" if referral_used else "wallet")
    )
    order = await database.get_order(order_id)
    try:
        await _fulfill_order(order, context)
        ...
        await database.refund_wallet_purchase(user_id, final_price, referral_used)
