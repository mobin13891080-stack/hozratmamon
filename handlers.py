...
async def send_main_menu(chat_id, context, user_id, account_name=None):
    menu_settings = await database.get_settings_many(
    ...
    # تنظیمات مستقل را موازی می‌خوانیم تا اولین نمایش منوی اصلی به چند درخواست پشت‌سرهم
    # به Turso وابسته نباشد؛ این فقط latency را کم می‌کند و هیچ ترتیب منطقی را تغییر نمی‌دهد.
