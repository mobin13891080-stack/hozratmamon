...
async def _apply_referral_penalty(context, referral, channel):
    ...
    try:
        text = ("⚠️ <b>جریمه خروج دعوت‌شده</b>\n━━━━━━━━━━━━\n"
                f"👤 دعوت‌شده: <code>{referral['referred_id']}</code>\n"
                f"📢 کانال لفت‌شده: <code>{html.escape(str(channel))}</code>\n\n")
        if cash_paid:
            text += f"💸 کسر از پاداش رفرال: <b>{cash_paid:,} تومان</b>\n"
        if service_changed:
            text += f"📦 کسر از سهم رفرالی سرویس: <b>{applied_gb:g} گیگ</b> و <b>{applied_days} روز</b>\n"
        if debt:
            text += f"🧾 بدهی جریمه رفرال: <b>{debt:,} تومان</b>\n"
        text += "\n🔒 موجودی و سهم خریداری‌شده با پول واقعی در این جریمه دستکاری نشده است."
