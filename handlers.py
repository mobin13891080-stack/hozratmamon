...
async def check_membership(context, channel, user_id):
    """بررسی عضویت fail-closed و سریع.

    هیچ لاگ DB برای هر کلیک/کاربر غیرعضو نوشته نمی‌شود؛ این لاگ‌نویسی قبلی می‌توانست
    خودِ مسیر Force Join را هنگام خطای عضویت/بار بالا کند کند. خطا فقط در logger محلی ثبت می‌شود.
    """
    try:
        member = await asyncio.wait_for(
            context.bot.get_chat_member(chat_id=channel, user_id=user_id), timeout=3.0
        )
...
async def get_missing_channels(context, user_id):
    ...
    return [channel for channel, is_member in zip(channels, results) if not is_member]
