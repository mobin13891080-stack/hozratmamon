...
async def send_main_menu(chat_id, context, user_id, account_name=None):
    ...
    photo_id, gif_id, is_admin_user, guide_value, referral_value = await asyncio.gather(
        database.get_setting("welcome_photo_id", ""),
        database.get_setting("welcome_gif_id", ""),
        database.is_admin(user_id),
        database.get_setting("connection_guide_enabled", "0"),
        database.get_setting("referral_enabled", "0"),
    )
...
async def start(update, context):
    ...
    _, referral_enabled = await asyncio.gather(
        database.upsert_user(user.id, user.username or ""),
        database.get_setting("referral_enabled", "0"),
    )
    # دریافت پارامتر لینک دعوت؛ فقط لینک‌های امضاشده نسخه ۵ پذیرفته می‌شوند.
    # لینک‌های قدیمی ref_<id> و هر payload جعلی عمداً نادیده گرفته می‌شوند.
