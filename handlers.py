...
# وضعیت عضویت اجباری عمداً کش نمی‌شود.
# چون کاربر می‌تواند در هر لحظه لفت بدهد، نتیجهٔ مثبت قبلی نباید حتی چند ثانیه هم
# دسترسی جعلی ایجاد کند. هر بار که قرار است خدمتی اجرا شود، وضعیت فعلی Telegram
# برای تمام کانال‌ها به‌صورت موازی و مستقیم بررسی می‌شود.


async def get_missing_channels(context, user_id):
    """عضویت فعلی کاربر را مستقیماً از Telegram می‌گیرد؛ بدون کش نتیجهٔ مثبت."""
    ...
    # تمام کانال‌ها هم‌زمان چک می‌شوند تا با زیاد شدن کانال‌ها سرعت افت نکند.
...
async def check_join_callback(update, context):
    query = update.callback_query
    # این callback هر بار یک بررسی واقعی و تازه از Telegram انجام می‌دهد.
