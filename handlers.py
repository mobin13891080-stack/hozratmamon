...
async def send_main_menu(chat_id, context, user_id, account_name=None):
    ...
    show_guide = await database.get_setting("connection_guide_enabled", "0") == "1"
    referral_enabled = await database.get_setting("referral_enabled", "0") == "1"
...
async def start(update, context):
    ...
    # لینک تست رفرال نسخه ۴: توکن تصادفی و فقط در تنظیمات سرور معتبر است. این مسیر در V5 حذف خواهد شد.
    if context.args:
        raw_arg = str(context.args[0]).strip()
        if raw_arg.startswith("testref_"):
            token = raw_arg[len("testref_"):]
            enabled = await database.get_setting("referral_test_enabled", "0") == "1"
            valid_token = await database.get_setting("referral_test_token", "")
            owner_id = int(await database.get_setting("referral_test_owner_id", "0") or 0)
            if enabled and token and valid_token and token == valid_token and owner_id > 0 and await database.is_admin(owner_id):
                reward = max(0, int(float(await database.get_setting("referral_reward", "5000") or 0)))
                credited = await database.add_referral_test_reward(owner_id, reward)
                await database.log_action("referral_test", owner_id, f"تست رفرال با کاربر {user.id} | پاداش {credited}")
                try:
                    await context.bot.send_message(
                        chat_id=owner_id,
                        text=f"🧪 <b>تست رفرال موفق</b>\n👤 اکانت تست: <code>{user.id}</code>\n💰 مبلغ اضافه‌شده: <b>{credited:,} تومان</b>",
                        parse_mode="HTML",
                    )
                except Exception:
                    pass
                await update.effective_message.reply_text("🧪 تست رفرال با موفقیت ثبت شد.")
                return
...
async def referral_show(update, context):
    ...
    if await database.get_setting("referral_enabled", "0") != "1":
        ...
        return
    code = await database.get_or_create_referral_code(user_id)
    try:
    ...
    summary = await database.referral_summary(user_id)
    reward = int(float(await database.get_setting("referral_reward", "5000") or 0))
    condition = await database.get_setting("referral_condition_enabled", "1") == "1"
