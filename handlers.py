...
async def _build_account_detail(order):
    ...
    # نمایش آخرین اتصال به صورت فارسی
    if online_at and online_at != "اطلاعاتی از پنل دریافت نشد":
        last_connection_display = str(online_at).replace("T", " ").split(".")[0]
    else:
        last_connection_display = "ثبت نشده"

    text = (
        f"{'━' * 15}\n"
        f"{status_icon} <b>وضعیت:</b> {status_label}\n"
        f"{'━' * 15}\n"
        f"👤 <b>نام سرویس:</b> <code>{username}</code>\n"
        f"📦 <b>پلن:</b> {product_name}\n"
        f"{price_line}"
        f"🔖 <b>شناسه سفارش:</b> #{order_id}\n"
        f"{'━' * 15}\n"
        f"📊 <b>مصرف ترافیک:</b>\n"
        f"{usage_bar}\n"
        f"📤 مصرف‌شده: <b>{used_str}</b>  |  📦 کل: <b>{limit_str}</b>\n"
        f"💾 باقی‌مانده: <b>{remaining_str}</b>{usage_note}\n"
        f"{'━' * 15}\n"
        f"{time_section}"
        f"📅 <b>تاریخ انقضا:</b> {expire_str}{countdown}{expiry_note}\n"
        f"🌐 <b>آخرین اتصال:</b> {last_connection_display}\n"
        f"{'━' * 15}\n"
        f"🔐 <b>امنیت:</b> اگر کسی دیگری از سرویست استفاده می‌کنه، «تعویض لینک» رو بزن تا دسترسیش قطع بشه."
    )
    action_rows = [
