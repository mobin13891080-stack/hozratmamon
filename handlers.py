...
async def check_membership(context, channel, user_id):
    ...
    منطق خطا:
    - Forbidden / ChatAdminRequired: بات admin کانال نیست → fail-OPEN (True)
      چون مشکل از بات هست نه از کاربر. اگه fail-closed کنیم همه کاربران فریز میشن.
    - Timeout: تلگرام کند هست → fail-OPEN (True) تا کاربر گیر نکنه
    - left / kicked / banned → False (واقعاً عضو نیست)
    """
    try:
        member = await asyncio.wait_for(
            context.bot.get_chat_member(chat_id=channel, user_id=user_id), timeout=2.5
        )
        return member.status in ("member", "administrator", "creator")
    except asyncio.TimeoutError:
        # تلگرام کند است — به کاربر شک نکن، رد بشه
        logger.warning("forcejoin timeout channel=%s user=%s — fail-open", channel, user_id)
        return True
    except Exception as error:
        err_str = str(error).lower()
        err_type = type(error).__name__
        # بات admin نیست یا به کانال دسترسی ندارد → مشکل از بات، نه کاربر
        if any(x in err_str for x in ("forbidden", "not enough rights", "chat admin", "bot is not", "kicked", "administrator")):
            logger.warning("forcejoin bot-not-admin channel=%s err=%s — fail-open", channel, err_type)
            return True
        # کانال وجود ندارد / اشتباه تایپ شده → fail-open تا همه گیر نکنن
        if any(x in err_str for x in ("chat not found", "invalid", "peer_id_invalid")):
            logger.warning("forcejoin invalid channel=%s err=%s — fail-open", channel, err_type)
            return True
        # سایر خطاها (network، etc) — fail-open
        logger.warning("forcejoin check failed channel=%s err=%s: %s — fail-open", channel, err_type, error)
        return True
...
# کش کوتاه ۳۰ ثانیه‌ای فقط برای نتیجه موفق (= عضو همه کانال‌ها)
# نتیجه منفی کش نمیشه — اگه کاربر لفت داد، اولین کلیک بعدیش رد میشه
import time as _time_module
_fj_ok_cache: dict = {}   # {user_id: timestamp}
_FJ_OK_TTL = 30.0         # ۳۰ ثانیه

async def get_missing_channels(context, user_id):
    """بررسی عضویت کاربر در کانال‌های اجباری با کش ۳۰ثانیه‌ای برای نتیجه موفق.
    
    بدون کش: هر کلیک دکمه = N تا getChatMember به تلگرام = کندی/freeze
    با کش: فقط هر ۳۰ ثانیه یه بار واقعاً چک میکنه، بقیه وقت‌ها از RAM جواب میده
    امنیت: نتیجه منفی (کانال باقیمانده) هرگز کش نمیشه
    """
    ...
    cached_ts = _fj_ok_cache.get(user_id)
    if cached_ts is not None and (now - cached_ts) < _FJ_OK_TTL:
        return []  # ۳۰ ثانیه پیش تأیید شده بود

    results = await asyncio.gather(
        *(check_membership(context, channel, user_id) for channel in channels),
        return_exceptions=False,
    )
    missing = [ch for ch, ok in zip(channels, results) if not ok]

    # فقط نتیجه کاملاً موفق رو کش کن
    if not missing:
        _fj_ok_cache[user_id] = now
...
async def check_join_callback(update, context):
    query = update.callback_query
    # پاک کردن کش تا چک واقعی انجام بشه
    _fj_ok_cache.pop(query.from_user.id, None)
