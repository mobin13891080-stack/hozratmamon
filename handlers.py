    ...
    try:
        ...
        try:
            await database.log_action(
                ...
                f"خطا در بررسی عضویت کانال {channel} برای کاربر {user_id}: {type(error).__name__}: {error} | نکته: ربات باید ادمین/مالک همین کانال باشد تا بتواند وضع����ت عضویت اعضا را ببیند",
            )
        except Exception:
            pass
        return False
...
async def _show_payment_choice(update_or_query, context, final_price, package=None, show_discount_option=True):
    ...
    wallet_line = f"💰 <b>موجود�� کیف پول شما:</b> {wallet_balance:,} تومان"
...
async def order_approve_callback(update, context):
    ...
    try:
        ...
        old_caption = query.message.caption or ""
        await _safe_edit_caption(query, old_caption + f"\n\n❌ <b>خطا در ساخت سرویس:</b>\n{html.escape(str(error))}")
        try:
...
async def _do_reject_order(query, context, order_id, order, reason=None):
    ...
    old_caption = query.message.caption or ""
    await _safe_edit_caption(query, old_caption + suffix)
...
async def _build_account_detail(order):
    ...
    toggle_label = "⛔️ خاموش کردن اکانت" if is_active else "✅ روشن کردن اکان��"
    action_rows = [
        [ibtn("♻️ بروزرسانی اطلاعات", callback_data=f"acc_refresh:{order_id}")],
