...
async def admin_wallet_manage_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("🔍 آیدی عددی یا یوزرنیم کاربر را وارد کنید: (لقو: /cancel)")
    return h.AWAITING_ADMIN_INPUT
...
async def admin_wallet_credit_callback(update, context):
    ...
    await query.answer()
    await query.message.reply_text("💵 مقدار افزایش موجودی (تومان) را وارد کنید: (لقو: /cancel)")
    return h.AWAITING_ADMIN_INPUT
...
async def admin_wallet_debit_callback(update, context):
    ...
    await query.answer()
    await query.message.reply_text("💵 مقدار کاهش موجودی (تومان) را وارد کنید: (لقو: /cancel)")
    return h.AWAITING_ADMIN_INPUT
