...
async def admin_finance_show(update, context):
    ...
    reset_total = await database.revenue_since_last_reset() if hasattr(database, "revenue_since_last_reset") else total
    last_reset = await database.last_revenue_reset() if hasattr(database, "last_revenue_reset") else None
    ...
    text = (
        ...
        f"♻️ درآمد از آخرین ریست: {reset_total:,} تومان\n"
        f"🕒 آخرین ریست: {(last_reset or {}).get('reset_at', '-') }\n"
        f"📅 امروز: {today:,} تومان\n"
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [h.ibtn("♻️ ریست شمارنده درآمد", callback_data="admin_revenue_reset", style="danger")],
    ])
...
async def admin_revenue_reset_callback(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "finance"):
        return
    total = await database.reset_revenue_counter(query.from_user.id)
    await query.answer("✅ ریست شد", show_alert=True)
    await query.message.reply_text(f"✅ شمارنده درآمد ریست شد. درآمد کل ثبت‌شده تا قبل از ریست: {total:,} تومان")
