...
async def admin_discounts_show(update, context):
    ...
    for c in codes:
        ...
        keyboard.append([InlineKeyboardButton(f"🗑 حذف {c['code']}", callback_data=f"discount_delete:{c['code']}")])
    keyboard.append([InlineKeyboardButton("➕ کد تخفیف جدید", callback_data="discount_add_start")])
    if not codes:
...
async def admin_servers_show(update, context):
    ...
    for s in servers:
        ...
        keyboard.append([
            InlineKeyboardButton("🔌 وضعیت", callback_data=f"server_check:{s['id']}"),
            InlineKeyboardButton("🔄 فعال/معلق", callback_data=f"server_toggle:{s['id']}"),
            InlineKeyboardButton("🗑", callback_data=f"server_delete:{s['id']}"),
        ])
    keyboard.append([InlineKeyboardButton("➕ افزودن سرور", callback_data="server_add_start")])
    if not servers:
...
async def admin_notify_show(update, context):
    ...
    keyboard = [[InlineKeyboardButton("📢 پیام همگانی به همه", callback_data="broadcast_start")]]
    for p in packages:
        keyboard.append([InlineKeyboardButton(f"🎯 به کاربران {p['name']}", callback_data=f"notify_pkg:{p['id']}")])
    await update.message.reply_text(
