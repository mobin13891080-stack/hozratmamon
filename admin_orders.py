...
async def _orders_page(offset, status_filter):
    ...
    nav_row = []
    if offset > 0:
        nav_row.append(InlineKeyboardButton("⬅️", callback_data=f"order_filter:{status_filter}:{max(offset - PAGE_SIZE, 0)}"))
    if offset + PAGE_SIZE < total:
        nav_row.append(InlineKeyboardButton("➡️", callback_data=f"order_filter:{status_filter}:{offset + PAGE_SIZE}"))
    if nav_row:
        keyboard.append(nav_row)
    keyboard.append([
        InlineKeyboardButton("⏳ در انتظار", callback_data="order_filter:pending:0"),
        InlineKeyboardButton("✅ تایید‌شده", callback_data="order_filter:approved:0"),
    ])
    keyboard.append([
        InlineKeyboardButton("❌ رد‌شده", callback_data="order_filter:rejected:0"),
        InlineKeyboardButton("🚫 لفو‌شده", callback_data="order_filter:cancelled:0"),
    ])
    keyboard.append([InlineKeyboardButton("📚 همه", callback_data="order_filter:all:0")])
...
async def order_action_callback(update, context):
    ...
    if data.startswith("order_view:"):
        ...
        lines = [
            ...
            f"📦 پلن: {package['name'] if package else '-'}",
        ...
        if order["status"] == "pending":
            keyboard.append([
                InlineKeyboardButton("✅ تایید", callback_data=f"order_approve:{order['id']}"),
                InlineKeyboardButton("❌ رد", callback_data=f"order_reject:{order['id']}"),
            ])
        if order["status"] == "approved":
            if order.get("marzban_username"):
                keyboard.append([InlineKeyboardButton("🔧 مدیریت اکانت", callback_data=f"mz_view:{order['marzban_username']}")])
            keyboard.append([InlineKeyboardButton("🚫 لفو سفارش", callback_data=f"order_cancel:{order['id']}")])
        keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data=f"order_filter:{status_filter}:0")])
    ...
    if data.startswith("order_cancel:"):
        ...
        await database.log_action("admin", query.from_user.id, f"لفو سفارش #{order_id}")
        await query.answer("سفارش لفو شد.")
        await query.message.reply_text("✅ سفارش لفو شد.")
        return
...
async def _mz_view_text_kb(username):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ افزایش حجم", callback_data=f"mz_incgb:{username}"),
         InlineKeyboardButton("📅 افزایش مدت", callback_data=f"mz_extend:{username}")],
        [InlineKeyboardButton("🔄 ریست مصرف", callback_data=f"mz_reset:{username}"),
         InlineKeyboardButton("⛔️ فعال/معلق", callback_data=f"mz_toggle:{username}")],
        [InlineKeyboardButton("🔗 لینک ساب", callback_data=f"mz_getlink:{username}")],
        [InlineKeyboardButton("🔔 اعلان تمدید", callback_data=f"mz_notify_renew:{username}"),
         InlineKeyboardButton("🔔 اتمام حجم", callback_data=f"mz_notify_data:{username}")],
        [InlineKeyboardButton("🗑 حذف اکانت", callback_data=f"mz_delete:{username}")],
    ])
...
async def mz_prompt_start(update, context):
    ...
    await query.answer()
    if action == "mz_incgb":
        await query.message.reply_text(f"مقدار گیگابایتی که می‌خواهید به {username} افزوده شود وارد کنید: (لغو: /cancel)")
    else:
        await query.message.reply_text(f"تعداد روزی که می‌خواهید به {username} افزوده شود وارد کنید: (لغو: /cancel)")
