...
async def _users_page(offset):
    ...
    keyboard = [[InlineKeyboardButton(_user_line(u), callback_data=f"user_view:{u['user_id']}")] for u in users]
    nav_row = []
    if offset > 0:
        nav_row.append(InlineKeyboardButton("⬅️ قبلی", callback_data=f"user_page:{max(offset - PAGE_SIZE, 0)}"))
    if offset + PAGE_SIZE < total:
        nav_row.append(InlineKeyboardButton("بعدی ➡️", callback_data=f"user_page:{offset + PAGE_SIZE}"))
    if nav_row:
        keyboard.append(nav_row)
    keyboard.append([InlineKeyboardButton("🔍 جستجوی کاربر", callback_data="user_search_start")])
...
async def user_search_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("🔍 ایدی عددی تلگرام یا نام کاربری (@username) را وارد کنید: (لغو: /cancel)")
...
async def broadcast_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("📢 متن پیام همگانی را وارد کنید: (لغو: /cancel)")
    return h.AWAITING_ADMIN_INPUT
...
async def notify_pkg_choose(update, context):
    ...
    await query.answer()
    await query.message.reply_text("🎯 متن پیام برای کاربران این پلن را وارد کنید: (لغو: /cancel)")
    return h.AWAITING_ADMIN_INPUT
...
async def _user_detail_view(user_id):
    ...
    for o in approved:
        if o.get("marzban_username"):
            keyboard.append([InlineKeyboardButton(f"🔧 مدیریت اکانت سفارش #{o['id']}", callback_data=f"mz_view:{o['marzban_username']}")])
    block_label = "✅ فعال‌سازی" if user["is_blocked"] else "🚫 مسدود سازی"
    keyboard.append([InlineKeyboardButton(block_label, callback_data=f"user_toggle_block:{user_id}")])
    keyboard.append([InlineKeyboardButton("📩 پیام خصوصی", callback_data=f"user_dm_start:{user_id}")])
    keyboard.append([InlineKeyboardButton("🗑 حذف کاربر", callback_data=f"user_delete:{user_id}")])
    keyboard.append([InlineKeyboardButton("🔙 بازگشت به لیست", callback_data="user_page:0")])
...
async def user_action_callback(update, context):
    ...
    if data.startswith("user_toggle_block:"):
        ...
        text, keyboard = await _user_detail_view(target_id)
        await query.edit_message_text(text, reply_markup=keyboard) if query.message.text else await query.message.reply_text(text, reply_markup=keyboard)
        return
...
async def user_dm_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text(f"📩 متن پیام خصوصی به {target_id} را وارد کنید: (لغو: /cancel)")
    return h.AWAITING_ADMIN_INPUT
