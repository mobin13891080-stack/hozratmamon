...
async def broadcast_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("📢 <b>متن پیام همگانی را وارد کنید:</b>", parse_mode="HTML", reply_markup=h.flow_cancel_keyboard())
    return h.AWAITING_ADMIN_INPUT
...
async def notify_pkg_choose(update, context):
    ...
    await query.answer()
    await query.message.reply_text("🎯 <b>متن پیام برای کاربران این پلن را وارد کنید:</b>", parse_mode="HTML", reply_markup=h.flow_cancel_keyboard())
    return h.AWAITING_ADMIN_INPUT
