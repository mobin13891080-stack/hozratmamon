...
async def broadcast_start(update, context):
    ...
    await query.message.reply_text(
        "📢 <b>پیام همگانی را ارسال کنید:</b>\n\nمتن، عکس، ویدئو، GIF، فایل، صدا، استیکر یا هر پیام دیگری را بفرستید؛ کپشن، قالب‌بندی و دکمه‌های همان پیام هم حفظ می‌شود.",
        parse_mode="HTML",
...
async def notify_pkg_choose(update, context):
    ...
    await query.answer()
    await query.message.reply_text(
        "🎯 <b>پیام برای کاربران این پلن را ارسال کنید:</b>\n\nهر نوع پیام (متن/عکس/ویدئو/GIF/فایل/صدا/استیکر و...) قابل ارسال است و کپشن/قالب‌بندی/دکمه‌ها حفظ می‌شوند.",
        parse_mode="HTML",
        reply_markup=h.flow_cancel_keyboard(),
    )
    return h.AWAITING_ADMIN_INPUT
