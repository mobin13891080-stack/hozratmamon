...
async def support_show(update, context):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎫 ایجاد تیکت", callback_data="ticket_new")],
        [InlineKeyboardButton("📋 تیکت‌های من", callback_data="ticket_list")],
        [InlineKeyboardButton("💬 تماس مستقیم", url=contact_url)],
    ])
...
async def ticket_new_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("مشکل یا سوال خود را در یک پیام بنویسید: (لغو: /cancel)")
...
async def receive_ticket_new_message(update, context):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("✍️ پاسخ", callback_data=f"ticket_reply_start:{ticket_id}")],
        [InlineKeyboardButton("✅ بستن تیکت", callback_data=f"ticket_close:{ticket_id}")],
    ])
...
async def ticket_view_callback(update, context):
    ...
    if ticket["user_id"] != query.from_user.id and not await database.is_admin(query.from_user.id):
        await query.answer("شما به این تیکت دسترسی ندارید.", show_alert=True)
        return
    await query.answer()
...
async def ticket_reply_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("متن پاسخ را وارد کنید: (لغو: /cancel)")
