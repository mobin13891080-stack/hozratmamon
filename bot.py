...
async def _global_access_precheck(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    از دکمه‌های منوی کلیک نمی‌کرد از جوین اجباری معاف می‌ماند. این تابع دکمه‌های شیشه‌ای را هم‌اکنون
    برای همه بررسی می‌کند تا تمام اعضا (فارق از اینکه قبلاً استارت کرده بودند یا نه) مجبور به جوین شوند."""
...
def build_application() -> Application:
    ...
    wallet_topup_conv = ConversationHandler(
        ...
        states={
            handlers.WALLET_TOPUP_AMOUNT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handlers.receive_wallet_topup_amount),
                CallbackQueryHandler(handlers.flow_cancel_callback, pattern="^flow_cancel$"),
            ],
            handlers.WALLET_TOPUP_RECEIPT: [
                MessageHandler(filters.PHOTO, handlers.receive_wallet_topup_receipt),
                CallbackQueryHandler(handlers.flow_cancel_callback, pattern="^flow_cancel$"),
            ],
        },
        fallbacks=[
    ...
    if application.job_queue is not None:
        ...
        logger.warning("JobQueue فعال نیست (پکیج job-queue نصب نشده)؛ حذف خودکار سرویس‌های منقضی غیرفعال است.")

    return application


async def _cleanup_expired_services(context: ContextTypes.DEFAULT_TYPE) -> None:
    """بررسی دوره‌ای سرویس‌های ساخته‌شده توسط خودِ ربات (خریداری‌شده یا تست رایگان) و حذف خودکار
    آن‌هایی که در پنل مرزبان منقضی/تمام‌شده‌اند - هم از دیتابیس (وضعیت expired که یعنی حذف از
    «سرویس‌های من») و هم از پنل مرزبان.
    نکته امنیتی مهم: این تابع فقط و فقط روی نام‌کاربری‌هایی کار می‌کند که در جدول orders خودِ ربات
    ثبت شده‌اند (یعنی سرویس‌هایی که خودِ ربات ساخته). کاربرهایی که ادمین مستقیماً و دستی در پنل
    مرزبان می‌سازد هرگز در این جدول ثبت نمی‌شوند، پس هرگز توسط این تابع بررسی یا حذف نخواهند شد."""
    try:
...
def main() -> None:
    if not config.BOT_TOKEN:
        raise RuntimeError("BOT_TOKEN تنزیم نشده است! لطفا این متفیر محیطی را در تنطیمات Render اضافه کنید.")
    ...
    if config.WEBHOOK_URL:
        ...
        logger.warning("WEBHOOK_URL تنطیم نشده; ربات با حالت polling (فقط برای تست محلی) اجرا می‌شود.")
