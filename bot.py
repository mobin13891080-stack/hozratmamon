...
async def _global_access_precheck(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async def _checks():
        is_admin, is_blocked, bot_enabled, rules_accepted = await asyncio.gather(
        ...
        if bot_enabled != "1":
            try: await query.answer("🪧 ربات موقتاً خاموش است. لطفا بعداً دوباره تلاش کنید.", show_alert=True)
            except Exception: pass
            raise ApplicationHandlerStop
        # بعد از عبور موفق از قوانین، برای هر کلیک دوباره API تلگرام را صدا نزن؛
        # پایش لفت رفرال جداگانه انجام می‌شود.
        if rules_accepted:
            return
