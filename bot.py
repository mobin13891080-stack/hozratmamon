...
async def _global_access_precheck(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    """گیت سبک دسترسی برای callbackها؛ هرگز نباید مسیر عادی ربات را معطل کند.

    نسخه‌های قبلی این گیت را برای *همه* پیام‌ها اجرا می‌کردند و در هر کلیک به Telegram
    برای Force-Join درخواست می‌زدند. اگر getChatMember یا Turso کند می‌شد، همین handler با
    ApplicationHandlerStop آپدیت را متوقف می‌کرد و کاربر هیچ پیامی نمی‌دید؛ نتیجه از دید کاربر
    دقیقاً «همه دکمه‌ها فریز شده‌اند» بود.

    از این نسخه به بعد: پیام‌های معمولی فقط توسط main_menu_router کنترل می‌شوند و این گیت فقط
    callbackها را پوشش می‌دهد. خطا/timeout سرویس‌های وابسته fail-open است تا outage شبکه باعث
    قفل شدن UX نشود. وضعیت قطعی blocked/bot-off/rules همچنان fail-closed می‌ماند.
    """
    ...
    async def _checks():
        ...
        if (settings.get("bot_enabled") or "1") != "1":
            try:
                await query.answer("🪧 ربات موقتاً خاموش است. لطفاً بعداً دوباره تلاش کنید.", show_alert=True)
            except Exception:
                pass
            raise ApplicationHandlerStop
        ...
        if missing:
            try:
                await query.answer("❌ عضویت شما کامل نیست؛ کانال‌های باقی‌مانده را ببینید.", show_alert=True)
            except Exception:
                pass
    ...
    try:
        await asyncio.wait_for(_checks(), timeout=2.5)
    except ApplicationHandlerStop:
        raise
    except asyncio.TimeoutError:
        logger.warning("access precheck timeout user=%s; allowing callback to continue", user_id)
        return
    except Exception:
        # یک outage در Turso/Telegram نباید کل callback router را از کار بیندازد.
        logger.exception("access precheck failed user=%s; allowing callback to continue", user_id)
        return
...
class _PerChatSerializingUpdateProcessor(BaseUpdateProcessor):
    ...
    def __init__(self, max_concurrent_updates: int = 64) -> None:
    ...
    async def do_process_update(self, update, coroutine) -> None:
        ...
        if elapsed >= 5.0:
            logger.warning(
                "slow update processing: %.2fs chat=%s update_id=%s",
                elapsed, key, getattr(update, "update_id", "?")
            )
...
def build_application() -> Application:
    ...
    application = (
        ...
        .concurrent_updates(_PerChatSerializingUpdateProcessor(max_concurrent_updates=64))
