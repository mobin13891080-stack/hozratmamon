...
def main() -> None:
    if not config.BOT_TOKEN:
        raise RuntimeError("BOT_TOKEN تنظیم نشده است! لطفا این متفیر محیطی را در تنطیمات Render اضافه کنید.")

    application = build_application()
    application.post_init = _post_init

    if config.WEBHOOK_URL:
        logger.info(f"ربات ��ر حالت Webhook روی پورت {config.PORT} اجرا می‌شود...")
