...
def _wire_main_menu_routes():
    ...
    handlers.MAIN_MENU_ROUTES.update({
        ...
        handlers.BTN_ADMIN_DEBUG: admin_stats_logs.admin_debug_show,
...
async def _global_access_precheck(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        await asyncio.wait_for(_checks(), timeout=7.0)
...
class _PerChatSerializingUpdateProcessor(BaseUpdateProcessor):
    ...
    def __init__(self, max_concurrent_updates: int = 128) -> None:
    ...
    async def do_process_update(self, update, coroutine) -> None:
        ...
        async with lock:
            await coroutine
        # عمداً lock را حذف نمی‌کنیم. حذف lock در همین لحظه یک race ظریف ایجاد می‌کند:
        # task دیگری ممکن است همین lock را گرفته باشد و task سوم lock جدید بسازد؛ در نتیجه
        # دو آپدیت از یک chat هم‌زمان اجرا می‌شوند و ConversationHandler دوباره خراب می‌شود.
        # تعداد lockها فقط به تعداد chatهای دیده‌شده است و برای این سرویس از memory leak واقعی
        # مهم‌تر، صحت سریال‌سازی است.
...
def build_application() -> Application:
    ...
    application = (
        ...
        .concurrent_updates(_PerChatSerializingUpdateProcessor(max_concurrent_updates=128))
    ...
    application.add_handler(CallbackQueryHandler(admin_stats_logs.debug_action_callback, pattern=r"^debug_action:\w+$"))
...
async def _run_webhook_server(application: Application) -> None:
    ...
    async def health_check_detailed(request: Request) -> PlainTextResponse:
        """وضعیت کامل ربات — برای debug دسترسی داشته باش"""
        lines = ["=== BOT STATUS ==="]
        # DB
        try:
            await asyncio.wait_for(database.execute("SELECT 1"), timeout=3.0)
            lines.append("DB: OK")
        except Exception as e:
            lines.append(f"DB: ERROR - {e}")
        # Panel
        try:
            ph = await asyncio.wait_for(marzban_api.panel_health(), timeout=6.0)
            lines.append(f"PANEL: OK (latency={ph.get('latency_ms')}ms groups={ph.get('groups_count')})")
        except Exception as e:
            lines.append(f"PANEL: ERROR - {e}")
        lines.append(f"PANEL_URL: {config.PANEL_URL}")
        lines.append(f"WEBHOOK: {config.WEBHOOK_URL}")
    ...
    starlette_app = Starlette(
        routes=[
            ...
            Route("/status", health_check_detailed, methods=["GET"]),
        ]
    )
    ...
    async with application:
        ...
        logger.info("📋 [CONFIG] BOT_TOKEN: %s", "✅ تنظیم شده" if config.BOT_TOKEN else "❌ خالی!")
        logger.info("📋 [CONFIG] ADMIN_ID: %s", config.ADMIN_ID or "❌ خالی!")
        logger.info("📋 [CONFIG] PANEL_URL: %s", config.PANEL_URL or "❌ خالی!")
        logger.info("📋 [CONFIG] TURSO_URL: %s", "✅ تنظیم شده" if config.TURSO_DATABASE_URL else "❌ خالی!")
        logger.info("📋 [CONFIG] WEBHOOK_URL: %s", config.WEBHOOK_URL or "❌ خالی!")

        # ۱. اتصال دیتابیس
        try:
            await database.init_db()
            logger.info("✅ [DB] دیتابیس Turso با موفقیت متصل و آماده شد.")
        except Exception as _db_err:
            logger.error("❌ [DB] خطا در اتصال به دیتابیس Turso: %s", _db_err)
            raise

        # ۲. تست پنل مرزبان — با get_token و سپس groups برای تست کامل
        try:
            _panel_health = await asyncio.wait_for(marzban_api.panel_health(), timeout=12.0)
            logger.info(
                "✅ [PANEL] پنل وصل شد: %s | latency=%dms | groups=%d",
                config.PANEL_URL,
                _panel_health.get("latency_ms", 0),
                _panel_health.get("groups_count", 0),
            )
            if _panel_health.get("groups_count", 0) == 0:
                logger.warning(
                    "⚠️ [PANEL] هیچ group ای در پنل پیدا نشد! "
                    "خرید و تست رایگان کار نمی‌کنند تا یه group فعال در پنل بسازی."
                )
        except asyncio.TimeoutError:
            logger.warning("⚠️ [PANEL] timeout در اتصال به پنل (%s) — ربات ادامه میده ولی خرید ممکنه کند باشه", config.PANEL_URL)
        except Exception as _panel_err:
            logger.warning("⚠️ [PANEL] پنل در دسترس نیست (%s): %s — ربات ادامه میده", config.PANEL_URL, _panel_err)

        # ۳. تست دسترسی به کانال‌های force_join
        try:
            import json as _json
            _fj_enabled = await database.get_setting("force_join_enabled", "0")
            _fj_raw = await database.get_setting("force_join_channels", "[]")
            _fj_channels = _json.loads(_fj_raw or "[]") if _fj_raw else []
            if _fj_enabled == "1" and _fj_channels:
                logger.info("🔍 [FORCEJOIN] در حال تست دسترسی به %d کانال...", len(_fj_channels))
                for _ch in _fj_channels:
                    try:
                        _chat = await asyncio.wait_for(
                            application.bot.get_chat(_ch), timeout=5.0
                        )
                        logger.info("  ✅ [FORCEJOIN] کانال %s — دسترسی OK (title: %s)", _ch, _chat.title or "?")
                    except asyncio.TimeoutError:
                        logger.warning("  ⚠️ [FORCEJOIN] timeout برای کانال %s — بررسی ادمین بودن بات ممکن نشد", _ch)
                    except Exception as _ch_err:
                        logger.error(
                            "  ❌ [FORCEJOIN] کانال %s — خطا: %s | "
                            "بات عضو/admin کانال نیست! همه کاربران برای خرید مشکل دارن!",
                            _ch, _ch_err
                        )
            else:
                logger.info("ℹ️ [FORCEJOIN] غیرفعال است.")
        except Exception as _fj_err:
            logger.warning("⚠️ [FORCEJOIN] خطا در تست کانال‌ها: %s", _fj_err)

        # ۴. تست봇 خود ربات
        try:
            _me = await asyncio.wait_for(application.bot.get_me(), timeout=5.0)
            logger.info("✅ [BOT] ربات @%s (id=%s) آنلاین است.", _me.username, _me.id)
        except Exception as _me_err:
            logger.error("❌ [BOT] خطا در دریافت اطلاعات ربات: %s", _me_err)

        # ۵. خلاصه نهایی
        logger.info("=" * 55)
        logger.info("✅ [READY] ربات آماده‌ست و webhook رو ست می‌کنه...")
        logger.info("=" * 55)
