...
async def _global_access_precheck(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    برای همه اعضا بررسی می‌کند تا تمام اعضا (فارق از اینکه قبلاً استارت کرده بودند یا نه) مجبور به جوین شوند."""
    ...
    purchase_conv = ConversationHandler(
        ...
        states={
            handlers.CUSTOM_GB_INPUT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handlers.receive_custom_gb),
                CallbackQueryHandler(handlers.flow_cancel_callback, pattern="^flow_cancel$"),
            ],
            handlers.CUSTOM_DAYS_INPUT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handlers.receive_custom_days),
                CallbackQueryHandler(handlers.flow_cancel_callback, pattern="^flow_cancel$"),
            ],
    ...
    admin_conv = ConversationHandler(
        ...
        states={
            handlers.AWAITING_ADMIN_INPUT: [
                ...
                MessageHandler(filters.ANIMATION, admin_core.admin_receive_animation_input),
