...
async def init_db():
    ...
    await _try_execute(client, "ALTER TABLE users ADD COLUMN referral_test_balance INTEGER DEFAULT 0")
    ...
    defaults = {
        ...
        "referral_test_token": "",
        "referral_test_owner_id": "0",
        "referral_test_enabled": "0",
        "referral_test_reconcile_done": "0",
    ...
    # V4 قبلی پاداش قفل‌شده را هنوز به wallet_balance اضافه نمی‌کرد. فقط یک‌بار اختلاف را
    # به موجودی کل و بخش قفل‌شده اضافه می‌کنیم تا مهاجرت داده بدون از دست رفتن موجودی انجام شود.
    if await get_setting("referral_test_reconcile_done", "0") != "1":
        rs = await execute(
            "SELECT referrer_id, COALESCE(SUM(MAX(reward_amount - COALESCE(released_amount,0),0)),0) AS locked "
            "FROM referrals WHERE COALESCE(is_test,0)=0 GROUP BY referrer_id"
        )
        for row in _rows_as_dicts(rs):
            referrer_id = int(row["referrer_id"] or 0)
            expected = int(row["locked"] or 0)
            if expected <= 0:
                continue
            cur = await execute("SELECT COALESCE(referral_locked_balance,0) FROM users WHERE user_id=?", [referrer_id])
            current_locked = int(cur.rows[0][0] or 0) if cur.rows else 0
            delta = max(0, expected - current_locked)
            if delta:
                await adjust_wallet_balance(referrer_id, delta)
                await execute("UPDATE users SET referral_locked_balance = COALESCE(referral_locked_balance,0)+? WHERE user_id=?", [delta, referrer_id])
        await set_setting("referral_test_reconcile_done", "1")
...
def make_referral_code(user_id: int) -> str:
    """کد پایدار و سازگار با داده‌های قدیمی؛ نیاز به جدول/نگاشت جداگانه ندارد."""
    return "ref_" + str(int(user_id))
...
async def get_or_create_referral_code(user_id: int) -> str:
    ...
    await execute("UPDATE users SET referral_code = ? WHERE user_id = ?", [code, user_id])
    return code
...
async def set_pending_referrer(user_id: int, referrer_id: int):
    ...
    if row.get("pending_referrer_id") or await get_referral_by_referred(user_id):
        return False
    await execute("UPDATE users SET pending_referrer_id = ? WHERE user_id = ?", [referrer_id, user_id])
    return True
...
async def release_referral_by_real_topup(user_id: int, amount: int):
    ...
    rs = await execute(
        ...
        "WHERE referrer_id = ? AND COALESCE(released_amount,0) < reward_amount "
...
async def qualify_referral_for_order(order_id: int):
    """خرید واجدشرایط دعوت واقعی یا تستی را ثبت می‌کند و در صورت تکمیل همه دعوت‌های همان دور، پاداش‌ها را آزاد می‌کند."""
    ...
    if not referral:
        rs = await execute(
            "SELECT * FROM referrals WHERE is_test=1 AND test_user_id=? AND COALESCE(qualified,0)=0 "
            "AND COALESCE(released_amount,0) < reward_amount ORDER BY id ASC LIMIT 1",
            [order["user_id"]],
        )
        rows = _rows_as_dicts(rs)
        referral = rows[0] if rows else None
    if not referral or referral.get("qualified"):
        return None

    await execute(
        "UPDATE referrals SET qualified = 1, qualified_order_id = ?, qualified_at = ? "
        "WHERE id = ? AND COALESCE(qualified,0)=0 AND COALESCE(released_amount,0) < reward_amount",
        [order_id, _now(), referral["id"]],
    )
    is_test = int(referral.get("is_test") or 0) == 1
    if is_test:
        scope = "is_test=1 AND test_owner_id=?"
    else:
        scope = "COALESCE(is_test,0)=0 AND referrer_id=?"
    rs = await execute(
        f"SELECT COUNT(*) AS c FROM referrals WHERE {scope} AND COALESCE(released_amount,0) < reward_amount AND qualified=0",
        [referral["test_owner_id"] if is_test else referral["referrer_id"]],
    )
...
async def release_all_locked_referrals():
    ...
    rs = await execute("SELECT id, referrer_id FROM referrals WHERE COALESCE(released_amount,0) < reward_amount")
    released = 0
    for row in _rows_as_dicts(rs):
...
def _new_test_token():
    import secrets
    return "rt4_" + secrets.token_urlsafe(24)


async def create_referral_test_link(owner_id: int, bot_username: str):
    token = _new_test_token()
    await set_setting("referral_test_token", token)
    await set_setting("referral_test_owner_id", str(int(owner_id)))
    await set_setting("referral_test_enabled", "1")
    return f"https://t.me/{bot_username}?start=testref_{token}"


async def disable_referral_test_link():
    await set_setting("referral_test_enabled", "0")
    await set_setting("referral_test_token", "")
    await set_setting("referral_test_owner_id", "0")


async def apply_referral_test_start(user_id: int, token: str):
    if await get_setting("referral_test_enabled", "0") != "1":
        return None
    valid_token = await get_setting("referral_test_token", "")
    owner = int(await get_setting("referral_test_owner_id", "0") or 0)
    if await get_setting("referral_enabled", "0") != "1":
        return None
    if not token or not valid_token or token != valid_token or owner <= 0 or not await is_admin(owner):
        return None
    reward = max(0, int(float(await get_setting("referral_reward", "5000") or 0)))
    condition = await get_setting("referral_condition_enabled", "1") == "1"
    status = "locked" if condition else "unlocked"
    # چون جدول referrals روی referred_id محدودیت UNIQUE دارد، برای تست هر Start یک شناسه منفی یکتا می‌سازیم؛
    # test_user_id شناسه واقعی اکانت تست است و برای واجدشرایط‌شدن خرید استفاده می‌شود.
    rs = await execute(
        "INSERT INTO referrals (referrer_id, referred_id, referral_code, reward_amount, status, qualified, created_at, is_test, test_user_id, test_owner_id) "
        "VALUES (?, ?, ?, ?, ?, 0, ?, 1, ?, ?)",
        [owner, -int(time.time_ns()), f"testref_{token}", reward, status, _now(), user_id, owner],
    )
    referral = await get_referral_by_id(rs.last_insert_rowid)
    if referral and reward > 0:
        await adjust_wallet_balance(owner, reward)
        if status == "unlocked":
            await execute("UPDATE users SET referral_available_balance = COALESCE(referral_available_balance,0) + ? WHERE user_id = ?", [reward, owner])
        else:
            await execute("UPDATE users SET referral_locked_balance = COALESCE(referral_locked_balance,0) + ? WHERE user_id = ?", [reward, owner])
        await execute("UPDATE users SET referral_test_balance = COALESCE(referral_test_balance,0) + ? WHERE user_id = ?", [reward, owner])
        await record_wallet_transaction(owner, reward, "referral_test_reward", note=f"پاداش تست رفرال #{referral['id']}")
    return referral


async def reset_referral_test_data(owner_id: int):
    rows = _rows_as_dicts(await execute("SELECT id FROM referrals WHERE is_test=1 AND test_owner_id=?", [owner_id]))
    tracked = await execute("SELECT COALESCE(referral_test_balance,0) FROM users WHERE user_id=?", [owner_id])
    test_balance = int(tracked.rows[0][0] or 0) if tracked.rows else 0
    wallet = await get_wallet_balance(owner_id)
    remove = min(max(test_balance, 0), max(wallet, 0))
    if remove:
        await adjust_wallet_balance(owner_id, -remove)
    await execute("UPDATE users SET referral_test_balance = 0 WHERE user_id=?", [owner_id])
    await execute("DELETE FROM referrals WHERE is_test=1 AND test_owner_id=?", [owner_id])
    if remove:
        await record_wallet_transaction(owner_id, -remove, "referral_test_reset", note="ریست موجودی تست رفرال نسخه ۴")
    return {"records": len(rows), "wallet_removed": remove}


async def reset_referral_test_balance(owner_id: int):
    result = await reset_referral_test_data(owner_id)
    return int(result.get("wallet_removed") or 0)
...
async def try_spend_wallet_for_purchase(user_id: int, amount: int, package_gb: float = 0):
    ...
    for _ in range(5):
        ...
        from_locked = remaining

        if from_locked > 0:
            if spend_limit_enabled and float(package_gb or 0) < min_gb:
                return False, -1
            if from_locked > referral_locked:
                return False, 0

        new_available = referral_available - from_available
        new_locked = referral_locked - from_locked
        try:
            upd = await execute(
