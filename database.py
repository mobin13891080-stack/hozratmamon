...
_SETTINGS_CACHE_MAX = 512
_SETTING_TTL  = 60.0   # Turso ابریه — ۶۰ ثانیه کش settings
_BLOCKED_CACHE_MAX = 4096
_BLOCKED_TTL  = 30.0   # وضعیت بلاک: ۳۰ ثانیه
_RULES_TTL    = 120.0  # قوانین: ۲ دقیقه
_ADMIN_TTL    = 60.0   # لیست ادمین: ۶۰ ثانیه
...
_seen_users: dict = {}  # {user_id: username} — کش in-memory تا ری‌استارت
_SEEN_USER_TTL = 300.0  # ۵ دقیقه — بعدش یه‌بار دیگه DB می‌زنه

async def upsert_user(user_id: int, username: str):
    """Create/update a user — فقط وقتی لازمه به Turso می‌زنه.
    اگه کاربر رو قبلاً با همین username دیدیم، DB call رو skip می‌کنیم."""
    now = time.time()
    username = username or ""
    cached = _seen_users.get(user_id)
    if cached is not None:
        cached_username, cached_time = cached
        if cached_username == username and (now - cached_time) < _SEEN_USER_TTL:
            return  # هیچ تغییری نیست — Turso call لازم نیست
    await execute(
    ...
    _seen_users[user_id] = (username, now)
...
# کش list_packages — پلن‌ها به ندرت تغییر می‌کنند، کش ۳۰ ثانیه خوانده رو خیلی کم می‌کنه
_list_packages_cache: dict = {}  # key: active_only bool → (result, timestamp)
_LIST_PACKAGES_TTL = 30.0

async def list_packages(active_only: bool = False) -> list:
    now = time.time()
    cached = _list_packages_cache.get(active_only)
    if cached is not None and (now - cached[1]) < _LIST_PACKAGES_TTL:
        return cached[0]
    if active_only:
    ...
    result = _rows_as_dicts(rs)
    _list_packages_cache[active_only] = (result, now)
    return result

def _invalidate_list_packages_cache():
    _list_packages_cache.clear()
...
def _invalidate_package_cache(package_id: int = None):
    """پاک‌سازی همه کش‌های مرتبط با پلن‌ها — بعد از هر تغییر ادمین صدا زده می‌شود."""
    if package_id is None:
        _package_cache.clear()
    else:
        _package_cache.pop(package_id, None)
    # کش‌های لیستی هم پاک می‌شوند تا تغییر ادمین فوری دیده شود
    _list_packages_cache.clear()
    _packages_by_cat_cache.clear()
...
_categories_cache = None
_categories_cache_time = 0.0
_CATEGORIES_TTL = 60.0  # دسته‌بندی‌ها خیلی کم تغییر می‌کنن — ۶۰ ثانیه

async def list_package_categories() -> list:
    global _categories_cache, _categories_cache_time
    now = time.time()
    if _categories_cache is not None and (now - _categories_cache_time) < _CATEGORIES_TTL:
        return _categories_cache
    rs = await execute("SELECT * FROM package_categories ORDER BY sort_order ASC, id ASC")
    _categories_cache = _rows_as_dicts(rs)
...
_packages_by_cat_cache: dict = {}
_PACKAGES_BY_CAT_TTL = 30.0

async def list_packages_by_category(category_id, active_only: bool = True) -> list:
    """category_id=None یعنی پلن‌های بدون دسته (سایر)"""
    cache_key = (category_id, active_only)
    now = time.time()
    cached = _packages_by_cat_cache.get(cache_key)
    if cached is not None and (now - cached[1]) < _PACKAGES_BY_CAT_TTL:
        return cached[0]
    ...
    result = _rows_as_dicts(rs)
    _packages_by_cat_cache[cache_key] = (result, time.time())
    return result
...
_wallet_cache: dict = {}
_WALLET_TTL = 10.0  # کوتاه — موجودی باید تازه باشه ولی هر کلیک نباید Turso بزنه

async def get_wallet_balance(user_id: int) -> int:
    now = time.time()
    cached = _wallet_cache.get(user_id)
    if cached is not None and (now - cached[1]) < _WALLET_TTL:
        return cached[0]
    rs = await execute("SELECT wallet_balance FROM users WHERE user_id = ?", [user_id])
    balance = 0 if (not rs.rows or rs.rows[0][0] is None) else int(rs.rows[0][0])
    _wallet_cache[user_id] = (balance, now)
    return balance

def _invalidate_wallet_cache(user_id: int):
...
async def adjust_wallet_balance(user_id: int, delta: int) -> int:
    ...
    _invalidate_wallet_cache(user_id)  # کش قدیمی پاک بشه تا موجودی جدید خونده بشه
