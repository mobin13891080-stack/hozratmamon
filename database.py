...
async def _try_execute(client, statement: str):
    ...
    «stuck» اگر ستون از قبل وجود دارد"""
    try:
...
async def init_db():
    ...
    # مهاجرت ستون‌های جدید برای دیتاباز‌های قدیمی‌تر (خطای «stuck» اگر قبلاً افزوده شده نادیده گرفته می‌شود)
...
async def list_logs(log_type: str = None, limit: int = 30) -> list:
    ...
    if log_type:
        ...
        rs = await client.execute("SELECT * FROM logs ORDER BY id DESC LIMIT ?", [limit])
...
async def get_admin_role(user_id: int) -> str:
    if user_id == config.ADMIN_ID:
        return "full"
    client = get_client()
    rs = await client.execute("SELECT role FROM admins WHERE user_id = ?", [user_id])
    if not rs.rows:
        return None
...
async def list_admins() -> list:
    client = get_client()
    rs = await client.execute("SELECT user_id, added_at, role FROM admins")
    return _rows_as_dicts(rs)
...
async def has_accepted_rules(user_id: int) -> bool:
    client = get_client()
    rs = await client.execute("SELECT rules_accepted FROM users WHERE user_id = ?", [user_id])
    if not rs.rows:
        return False
    return bool(rs.rows[0][0])
...
async def mark_rules_accepted(user_id: int):
    client = get_client()
    await client.execute("UPDATE users SET rules_accepted = 1 WHERE user_id = ?", [user_id])


async def is_user_blocked(user_id: int) -> bool:
    client = get_client()
    rs = await client.execute("SELECT is_blocked FROM users WHERE user_id = ?", [user_id])
    if not rs.rows:
        return False
    return bool(rs.rows[0][0])
...
async def set_user_blocked(user_id: int, blocked: bool):
    client = get_client()
    await client.execute("UPDATE users SET is_blocked = ? WHERE user_id = ?", [1 if blocked else 0, user_id])
...
async def search_users(query: str) -> list:
    ...
    if query.isdigit():
        rs = await client.execute("SELECT * FROM users WHERE user_id = ?", [int(query)])
    else:
        rs = await client.execute("SELECT * FROM users WHERE username LIKE ?", [f"%{query}%"])
    return _rows_as_dicts(rs)
...
async def count_active_users() -> int:
    """کاربرانی که حداقل یک سفارش تاییدشده دارند"""
    client = get_client()
    rs = await client.execute(
...
async def create_order(user_id: int, package_id: int, receipt_file_id: str, price: int = None, discount_code: str = None) -> int:
    client = get_client()
    rs = await client.execute(
        "INSERT INTO orders (user_id, package_id, status, receipt_file_id, created_at, price, discount_code) "
        "VALUES (?, ?, 'pending', ?, ?, ?, ?)",
        [user_id, package_id, receipt_file_id, _now(), price, discount_code],
    )
    return rs.last_insert_rowid
...
async def count_orders(status: str = None) -> int:
    client = get_client()
    if status:
        rs = await client.execute("SELECT COUNT(*) FROM orders WHERE status = ?", [status])
    else:
        rs = await client.execute("SELECT COUNT(*) FROM orders")
    return rs.rows[0][0]
...
async def revenue_since(iso_date: str) -> int:
    client = get_client()
    rs = await client.execute(
        "SELECT COALESCE(SUM(price), 0) FROM orders WHERE status = 'approved' AND datetime(created_at) >= datetime(?)",
        [iso_date],
    )
    return rs.rows[0][0] or 0
...
async def get_discount_code(code: str):
    client = get_client()
    rs = await client.execute("SELECT * FROM discount_codes WHERE code = ?", [code])
    rows = _rows_as_dicts(rs)
    return rows[0] if rows else None
...
async def list_discount_codes() -> list:
    client = get_client()
    rs = await client.execute("SELECT * FROM discount_codes ORDER BY created_at DESC")
    return _rows_as_dicts(rs)


async def delete_discount_code(code: str):
    client = get_client()
    await client.execute("DELETE FROM discount_codes WHERE code = ?", [code])
...
async def list_servers() -> list:
    client = get_client()
    rs = await client.execute("SELECT * FROM servers ORDER BY id ASC")
    return _rows_as_dicts(rs)


async def get_server(server_id: int):
    client = get_client()
    rs = await client.execute("SELECT * FROM servers WHERE id = ?", [server_id])
    rows = _rows_as_dicts(rs)
    return rows[0] if rows else None
...
async def remove_server(server_id: int):
    client = get_client()
    await client.execute("DELETE FROM servers WHERE id = ?", [server_id])
...
async def get_ticket(ticket_id: int):
    client = get_client()
    rs = await client.execute("SELECT * FROM tickets WHERE id = ?", [ticket_id])
    rows = _rows_as_dicts(rs)
    return rows[0] if rows else None
...
async def set_ticket_status(ticket_id: int, status: str):
    client = get_client()
    await client.execute("UPDATE tickets SET status = ? WHERE id = ?", [status, ticket_id])
...
async def get_wallet_balance(user_id: int) -> int:
    client = get_client()
    rs = await client.execute("SELECT wallet_balance FROM users WHERE user_id = ?", [user_id])
...
async def get_wallet_transaction(tx_id: int):
    client = get_client()
    rs = await client.execute("SELECT * FROM wallet_transactions WHERE id = ?", [tx_id])
    rows = _rows_as_dicts(rs)
    return rows[0] if rows else None
...
async def update_wallet_transaction(tx_id: int, **fields):
    ...
    client = get_client()
    await client.execute(f"UPDATE wallet_transactions SET {columns} WHERE id = ?", values)
