...
async def get_referral_by_referred(referred_id: int):
    rs = await execute("SELECT * FROM referrals WHERE referred_id = ? LIMIT 1", [referred_id])
    rows = _rows_as_dicts(rs)
    return rows[0] if rows else None
...
async def finalize_pending_referral(referred_id: int):
    ...
    if referral and referral["status"] == "unlocked":
        await adjust_wallet_balance(referral["referrer_id"], referral["reward_amount"])
        await execute(
        ...
        await record_wallet_transaction(
            referral["referrer_id"], referral["reward_amount"], "referral_reward",
            note=f"پاداش دعوت موفق #{referral['id']}",
        )
    return referral
...
async def unlock_referral_reward(referral_id: int, order_id: int):
    ...
    try:
        ...
        if not changed or int(changed.get("released_amount") or 0) < int(changed.get("reward_amount") or 0):
            return False
        credit = remaining
    await adjust_wallet_balance(changed["referrer_id"], credit)
    await execute("UPDATE users SET referral_available_balance = COALESCE(referral_available_balance, 0) + ? WHERE user_id = ?", [credit, changed["referrer_id"]])
    await record_wallet_transaction(
        changed["referrer_id"], credit, "referral_reward",
...
async def release_referral_by_real_topup(user_id: int, amount: int):
    ...
    if total_released:
        await adjust_wallet_balance(user_id, total_released)
        await execute("UPDATE users SET referral_available_balance = COALESCE(referral_available_balance,0) + ? WHERE user_id = ?", [total_released, user_id])
...
async def qualify_referral_for_order(order_id: int):
    """خرید واجدشرایط دوست، وضعیت همان دعوت را ثبت می‌کند؛ وقتی همه دعوت‌های فعلی واجدشرایط شدند،
    کل پاداش‌های قفل‌شده آن معرف یکجا آزاد می‌شوند."""
    ...
    # ابتدا فقط همین دعوت را qualified می‌کنیم؛ هنوز کیف پول شارژ نمی‌شود.
    await execute(
        "UPDATE referrals SET qualified = 1, qualified_order_id = ?, qualified_at = ? "
        "WHERE id = ? AND status = 'locked' AND qualified = 0",
        [order_id, _now(), referral["id"]],
    )
    # تا وقتی حتی یک دعوت قفل‌شده و تأییدنشده برای این معرف باقی است، هیچ پاداشی آزاد نمی‌شود.
    rs = await execute(
        "SELECT COUNT(*) AS c FROM referrals WHERE referrer_id = ? AND status = 'locked' AND qualified = 0",
    ...
    # همه دعوت‌های ثبت‌شده این معرف واجدشرایط شده‌اند؛ پاداش‌های این دور یکجا آزاد می‌شوند.
    rs = await execute(
        "SELECT id FROM referrals WHERE referrer_id = ? AND status = 'locked' AND qualified = 1",
        [referral["referrer_id"]],
    )
    unlocked = 0
    for row in _rows_as_dicts(rs):
...
async def get_referral_admin_rows(limit: int = 10, offset: int = 0):
    rs = await execute(
        ...
        "ORDER BY r.id DESC LIMIT ? OFFSET ?", [int(limit), int(offset)]
    )
    return _rows_as_dicts(rs)
...
async def get_referral_admin_detail(referral_id: int):
    rs = await execute(
        ...
        "FROM referrals r LEFT JOIN users u ON u.user_id=r.referred_id "
        "LEFT JOIN users u2 ON u2.user_id=r.referrer_id WHERE r.id=? LIMIT 1", [referral_id]
    )
    rows = _rows_as_dicts(rs)
    return rows[0] if rows else None
...
async def reset_referral_record(referral_id: int):
    ...
    if not row:
        return False
    await execute("DELETE FROM referrals WHERE id = ?", [referral_id])
    return True
...
async def apply_referral_test_start(user_id: int):
    if await get_setting("referral_test_enabled", "0") != "1":
        return None
    token = await get_setting("referral_test_token", "")
    owner = int(await get_setting("referral_test_owner_id", "0") or 0)
    if not token or owner <= 0:
        return None
    return {"owner_id": owner, "reward": max(0, int(float(await get_setting("referral_reward", "5000") or 0)))}


async def add_referral_test_reward(owner_id: int, amount: int):
    amount = max(0, int(amount or 0))
    if amount <= 0:
        return 0
    await adjust_wallet_balance(owner_id, amount)
    await execute("UPDATE users SET referral_test_balance = COALESCE(referral_test_balance,0) + ? WHERE user_id = ?", [amount, owner_id])
    await record_wallet_transaction(owner_id, amount, "referral_test_reward", note="پاداش تست رفرال نسخه ۴")
    return amount


async def reset_referral_test_balance(owner_id: int):
    rs = await execute("SELECT COALESCE(referral_test_balance,0) FROM users WHERE user_id=?", [owner_id])
    amount = int(rs.rows[0][0] or 0) if rs.rows else 0
    if amount <= 0:
        return 0
    # فقط مقدار تستیِ باقی‌مانده کم می‌شود؛ اگر بخشی قبلاً خرج شده، موجودی واقعی ادمین دستکاری نمی‌شود.
    current = await get_wallet_balance(owner_id)
...
async def try_spend_wallet_for_purchase(user_id: int, amount: int, package_gb: float = 0):
    """پرداخت از همان کیف پول کاربر، با تفکیک داخلی منبع پاداش رفرال.
    کاربر فقط یک موجودی می‌بیند؛ تفکیک صرفاً برای اعمال محدودیت حداقل حجم روی بخش رفرالی است.
    خروجی: (success, referral_used).
    """
    if amount <= 0:
        return True, 0
    ...
    for _ in range(4):
        rs = await execute(
            "SELECT COALESCE(wallet_balance,0), COALESCE(referral_available_balance,0) "
            "FROM users WHERE user_id = ?", [user_id]
        )
        if not rs.rows:
            return False, 0
        wallet, referral_part = int(rs.rows[0][0] or 0), int(rs.rows[0][1] or 0)
        if wallet < amount:
            return False, 0
        referral_part = min(max(referral_part, 0), wallet)
        regular_part = wallet - referral_part
        referral_used = max(0, amount - regular_part)
        if referral_used > 0 and spend_limit_enabled and float(package_gb or 0) < min_gb:
            return False, -1

        new_referral = referral_part - referral_used
        try:
            upd = await execute(
                "UPDATE users SET wallet_balance = wallet_balance - ?, "
                "referral_available_balance = ? "
                "WHERE user_id = ? AND COALESCE(wallet_balance,0) = ? "
                "AND COALESCE(referral_available_balance,0) = ? RETURNING user_id",
                [amount, new_referral, user_id, wallet, referral_part],
            )
            if upd.rows:
                return True, referral_used
        except Exception:
            pass
        # مسیر سازگار با libSQL/SQLite بدون RETURNING
        await execute(
            "UPDATE users SET wallet_balance = wallet_balance - ?, referral_available_balance = ? "
            "WHERE user_id = ? AND COALESCE(wallet_balance,0) = ? "
            "AND COALESCE(referral_available_balance,0) = ?",
            [amount, new_referral, user_id, wallet, referral_part],
        )
        after = await execute(
            "SELECT COALESCE(wallet_balance,0), COALESCE(referral_available_balance,0) "
        ...
        if after.rows and int(after.rows[0][0] or 0) == wallet - amount and int(after.rows[0][1] or 0) == new_referral:
            return True, referral_used
    return False, 0
