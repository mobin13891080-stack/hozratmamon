...
async def init_db():
    ...
    await _try_execute(client, "ALTER TABLE users ADD COLUMN trial_gen INTEGER DEFAULT 0")
...
async def clear_logs(log_type: str = None) -> int:
    """حذف لاگ‌ها. اگر log_type داده شود فقط همان دسته، واگرنه همهٔ لاگ‌ها حذف می‌شوند.
    این تابع فقط جدول logs را دستکاری می‌کند و هیچ ربطی به کاربران/سفارش‌ها ندارد."""
    if log_type and log_type != "all":
        rs = await execute("SELECT COUNT(*) FROM logs WHERE log_type = ?", [log_type])
        affected = int(rs.rows[0][0]) if rs.rows else 0
...
async def has_used_trial(user_id: int) -> bool:
    """کاربر اگر used_trial=1 باشد و trial_gen او با نسل فعلی trial_generation یکسان باشد، قبلاً تست گرفته است.
    با زدن دکمهٔ «ریست تست»، نسل فعلی یک‌واحد بالا می‌رود و همه یک‌بار دیگر مجاز می‌شوند."""
    rs = await execute("SELECT used_trial, trial_gen FROM users WHERE user_id = ?", [user_id])
    if not rs.rows:
        return False
    used_trial, trial_gen = rs.rows[0][0], rs.rows[0][1]
    if not used_trial:
        return False
    current_gen = int(await get_setting("trial_generation", "1") or "1")
    return int(trial_gen or 0) >= current_gen


async def mark_trial_used(user_id: int):
    current_gen = int(await get_setting("trial_generation", "1") or "1")
    await execute("UPDATE users SET used_trial = 1, trial_gen = ? WHERE user_id = ?", [current_gen, user_id])


async def reset_trial_all() -> int:
    """به همهٔ کاربرانی که در نسل فعلی مسدود هستند، یک بار دیگر امکان دریافت تست می‌دهد
    (بدون حذف هیچ داده‌ای کاربر). روش کار: نسل فعلی trial_generation یکی افزایش می‌یابد."""
    current_gen = int(await get_setting("trial_generation", "1") or "1")
    rs = await execute("SELECT COUNT(*) FROM users WHERE used_trial = 1 AND COALESCE(trial_gen, 0) >= ?", [current_gen])
    affected = int(rs.rows[0][0]) if rs.rows else 0
    await set_setting("trial_generation", str(current_gen + 1))
    return affected
...
async def create_order(user_id: int, package_id, receipt_file_id, price: int = None, discount_code: str = None, order_type: str = "purchase", custom_gb: float = None, custom_days: int = None, desired_username: str = None) -> int:
    client = get_client()
    rs = await execute(
        "INSERT INTO orders (user_id, package_id, status, receipt_file_id, created_at, price, discount_code, order_type, custom_gb, custom_days, desired_username) "
        "VALUES (?, ?, 'pending', ?, ?, ?, ?, ?, ?, ?, ?)",
        [user_id, package_id, receipt_file_id, _now(), price, discount_code, order_type, custom_gb, custom_days, desired_username],
    )
    return rs.last_insert_rowid
...
async def revenue_total() -> int:
    rs = await execute(
        ...
        [await get_setting("revenue_reset_at", "1970-01-01T00:00:00")],
    )
    return rs.rows[0][0] or 0


async def reset_revenue() -> None:
    await set_setting("revenue_reset_at", _now())
