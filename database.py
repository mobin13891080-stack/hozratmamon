...
async def referral_summary(referrer_id: int):
    rs = await execute(
        ...
        "COALESCE(SUM(CASE WHEN status = 'unlocked' THEN reward_amount ELSE 0 END), 0) AS unlocked, "
        "COALESCE(SUM(CASE WHEN status = 'locked' THEN reward_amount ELSE 0 END), 0) AS locked "
...
async def unlock_referral_reward(referral_id: int, order_id: int):
    ...
    if not referral or referral["status"] == "unlocked":
        return False
    # UPDATE اتمیک: فقط اولین پردازش‌کننده‌ای که وضعیت locked را می‌بیند برنده می‌شود.
    try:
        rs = await execute(
            "UPDATE referrals SET status = 'unlocked', qualified = 1, qualified_order_id = ?, qualified_at = ? "
            "WHERE id = ? AND status = 'locked' RETURNING id, referrer_id, reward_amount",
        ...
        # سازگاری با نسخه‌های SQLite/libSQL که RETURNING را در مسیر فعلی در دسترس ندارند.
        ...
        if not changed or changed["status"] != "unlocked":
            return False
    await adjust_wallet_balance(changed["referrer_id"], changed["reward_amount"])
    await execute("UPDATE users SET referral_available_balance = COALESCE(referral_available_balance, 0) + ? WHERE user_id = ?", [changed["reward_amount"], changed["referrer_id"]])
    await record_wallet_transaction(
        changed["referrer_id"], changed["reward_amount"], "referral_reward",
...
async def release_all_locked_referrals():
    """وقتی شرط خاموش شد، پاداش‌های قفل‌شده قدیمی نیز قابل استفاده می‌شوند."""
    rs = await execute("SELECT id, referrer_id, reward_amount FROM referrals WHERE status = 'locked'")
    released = 0
    for row in _rows_as_dicts(rs):
        rid = row["id"]
        await execute("UPDATE referrals SET status = 'unlocked', qualified = 1, qualified_at = ? WHERE id = ? AND status = 'locked'", [_now(), rid])
        check = await get_referral_by_id(rid)
        if check and check["status"] == "unlocked":
            await adjust_wallet_balance(check["referrer_id"], check["reward_amount"])
            await execute("UPDATE users SET referral_available_balance = COALESCE(referral_available_balance, 0) + ? WHERE user_id = ?", [check["reward_amount"], check["referrer_id"]])
            await record_wallet_transaction(check["referrer_id"], check["reward_amount"], "referral_reward",
                                            note=f"آزادسازی پاداش رفرال #{rid} پس از غیرفعال شدن شرط")
            released += 1
    return released
