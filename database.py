...
def _referral_secret() -> bytes:
    # امضای لینک فقط از BOT_TOKEN ساخته می‌شود؛ خود توکن هرگز داخل لینک یا دیتابیس ذخیره نمی‌شود.
...
def make_referral_code(user_id: int) -> str:
    """Deep-link امضاشده و غیرقابل جعل؛ فرمت فقط از کاراکترهای مجاز Telegram تشکیل شده است."""
    uid = str(int(user_id))
    sig = hmac.new(_referral_secret(), ("ref:" + uid).encode("utf-8"), hashlib.sha256).digest()[:12]
    sig_text = sig.hex()
    return "r1_" + uid + "_" + sig_text


def resolve_referral_code(code: str):
    """کد رفرال را بدون هیچ اعتماد به user input اعتبارسنجی می‌کند."""
    if not code or not isinstance(code, str) or not code.startswith("r1_"):
        return None
    parts = code.split("_")
...
async def get_or_create_referral_code(user_id: int) -> str:
    ...
    if row and row.get("referral_code") and resolve_referral_code(str(row["referral_code"])) == int(user_id):
