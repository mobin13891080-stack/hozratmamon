...
async def create_order(user_id: int, package_id, receipt_file_id, price: int = None, discount_code: str = None, order_type: str = "purchase", custom_gb: float = None, custom_days: int = None) -> int:
    client = get_client()
    rs = await execute(
        "INSERT INTO orders (user_id, package_id, status, receipt_file_id, created_at, price, discount_code, order_type, custom_gb, custom_days) "
        "VALUES (?, ?, 'pending', ?, ?, ?, ?, ?, ?, ?)",
        [user_id, package_id, receipt_file_id, _now(), price, discount_code, order_type, custom_gb, custom_days],
    )
    return rs.last_insert_rowid
...
async def try_spend_wallet_balance(user_id: int, amount: int) -> bool:
    ...
    فقط وقتی موجودی کافی باشد کاهش انجام می‌شود و True برمی‌گردد؛ در فیر این صورت هیچ تفییری
