...
def _from_base36(value: str) -> int:
    if not value or not all(c.isdigit() or c.islower() for c in value):
        raise ValueError("invalid base36")
    return int(value, 36)


def make_referral_code(user_id: int) -> str:
    """Short signed Telegram deep-link: r2-<base36 uid>-<11 char signature>."""
    uid = _to_base36(int(user_id))
    raw = "ref2:" + str(int(user_id))
    sig = hmac.new(_referral_secret(), raw.encode("utf-8"), hashlib.sha256).digest()[:8]
    sig_text = __import__("base64").urlsafe_b64encode(sig).decode("ascii").rstrip("=")
    return "r2-" + uid + "-" + sig_text


def resolve_referral_code(code: str):
    """Validate both the compact V9 code and legacy signed V5/V8 codes."""
    ...
    if code.startswith("r2-"):
        ...
        if len(parts) != 3 or not parts[1] or not parts[2]:
            return None
        try:
            uid = _from_base36(parts[1])
        except Exception:
            return None
        expected = make_referral_code(uid)
        return uid if hmac.compare_digest(code, expected) else None
    if code.startswith("r1_"):
        parts = code.split("_")
        if len(parts) != 3 or not parts[1].isdigit() or not parts[2]:
            return None
        uid = int(parts[1])
        raw = "ref:" + str(uid)
        sig = hmac.new(_referral_secret(), raw.encode("utf-8"), hashlib.sha256).digest()[:12]
        expected = "r1_" + str(uid) + "_" + sig.hex()
        return uid if hmac.compare_digest(code, expected) else None
    return None
...
async def get_or_create_referral_code(user_id: int) -> str:
    ...
    if row and row.get("referral_code") and str(row["referral_code"]).startswith("r2-"):
        if resolve_referral_code(str(row["referral_code"])) == int(user_id):
...
async def set_pending_referrer(user_id: int, referrer_id: int):
    ...
    if row.get("pending_referrer_id") or await get_referral_by_referred(user_id):
        return False
    await execute("UPDATE users SET pending_referrer_id = ? WHERE user_id = ? AND pending_referrer_id IS NULL", [referrer_id, user_id])
    return True
...
async def list_referral_penalty_candidates(limit: int = 50):
    rs = await execute(
        "SELECT * FROM referrals WHERE COALESCE(is_test,0)=0 AND (COALESCE(left_at,'')='' OR COALESCE(penalty_status,'none')='pending') "
        "ORDER BY CASE WHEN COALESCE(penalty_status,'none')='pending' THEN 0 ELSE 1 END, COALESCE(penalty_checked_at,'') ASC, id ASC LIMIT ?", [int(limit)]
    )
    return _rows_as_dicts(rs)


async def claim_referral_leave(referral_id: int, channel: str):
    """یک لفت تاییدشده را فقط یک بار claim می‌کند؛ اگر دو worker همزمان اجرا شوند، فقط یکی موفق می‌شود."""
    try:
        rs = await execute(
            "UPDATE referrals SET left_at=?, left_channel=?, penalty_status=CASE WHEN COALESCE(penalty_status,'none')='none' THEN 'pending' ELSE penalty_status END "
            "WHERE id=? AND COALESCE(left_at,'')='' RETURNING id, referrer_id, referred_id, reward_amount, released_amount, penalty_status",
            [_now(), str(channel)[:300], referral_id],
        )
        rows = _rows_as_dicts(rs)
        return rows[0] if rows else None
    except Exception:
        await execute(
            "UPDATE referrals SET left_at=?, left_channel=?, penalty_status=CASE WHEN COALESCE(penalty_status,'none')='none' THEN 'pending' ELSE penalty_status END "
            "WHERE id=? AND COALESCE(left_at,'')=''", [_now(), str(channel)[:300], referral_id]
        )
        row = await get_referral_by_id(referral_id)
        return row if row and row.get("left_at") else None


async def mark_referral_penalty_checked(referral_id: int):
    await execute("UPDATE referrals SET penalty_checked_at=? WHERE id=? AND COALESCE(left_at,'')=''", [_now(), referral_id])
