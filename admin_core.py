...
async def admin_receive_forward_input(update, context):
    ...
    from_chat_id = message.forward_from_chat.id if message.forward_from_chat else None
    message_id = message.forward_from_message_id if message.forward_from_chat else None
...
async def _pkg_template_menu_view():
    current = await database.get_setting("package_label_template", h.DEFAULT_PACKAGE_LABEL_TEMPLATE)
    lines = [
        "🎨 <b>قالب نمایش پلن‌ها</b>",
        "━━━━━━━━━━━",
        "پیش‌نمایش با یک پلن نمونه (150 گیگ/2 ماهه/444,000 تومان):",
        "",
    ]
    keyboard = []
    for idx, tpl in enumerate(h.PACKAGE_LABEL_TEMPLATES):
        ...
        keyboard.append([h.ibtn(f"انتخاب قالب {idx + 1}", callback_data=f"admin_pkg_template_set:{idx}")])
    keyboard.append([h.ibtn("✏️ قالب دلخواه", callback_data="admin_set_package_template")])
...
async def admin_pkg_template_set_callback(update, context):
    ...
    await database.set_setting("package_label_template", template)
    await database.log_action("admin", query.from_user.id, f"انتخاب قالب نمایش پلن #{idx + 1}")
...
async def _buy_prompt_menu_view():
    ...
    lines = [
        ...
        f"متن فعلی: {current}",
        "",
        "یکی از متن‌های جذاب زیر را انتخاب کنید:",
        "",
    ]
    keyboard = []
...
async def admin_receive_input(update, context):
    ...
    # قالب دلخواه نمایش پلن (نیاز به اعتبارسنجی خاص)
    if action == "admin_set_package_template":
        ...
        if preview is None:
            ...
            return h.AWAITING_ADMIN_INPUT
        await database.set_setting("package_label_template", text)
        await database.log_action("admin", update.effective_user.id, "قالب دلخواه نمایش پلن ثبت شد")
        await update.message.reply_text(f"✅ قالب ذخیره شد.\n\nپیش‌نمایش:\n{preview}")
        context.user_data.pop("admin_action", None)
        return ConversationHandler.END
