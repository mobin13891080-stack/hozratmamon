...
async def admin_referral_settings_show(update, context):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [h.ibtn("🧪 لینک تست رفرال (نسخه ۴)", callback_data="admin_referral_test_link")],
        [h.ibtn("🛑 غیرفعال کردن لینک تست", callback_data="admin_referral_test_disable")],
        [h.ibtn("♻️ ریست موجودی تست رفرال", callback_data="admin_referral_test_reset")],
...
async def admin_referral_test_link(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    try:
        await query.answer("در حال ساخت لینک تست...")
    except Exception:
        pass
    try:
        me = await context.bot.get_me()
        link = await database.create_referral_test_link(query.from_user.id, me.username or "")
        text = (
            "🧪 <b>لینک تست رفرال نسخه ۴</b>\n"
            "━━━━━━━━━━━━━━━━━━\n"
            f"🔗 <code>{link}</code>\n\n"
            "هر اکانتی این لینک را Start کند، یک <b>رکورد رفرال تستی واقعی</b> برای شما ثبت و پاداش با مبلغ فعلی رفرال در کیف پول شما ثبت می‌شود؛ حتی اگر آن اکانت قبلاً ربات را Start کرده یا رفرال دیگری داشته باشد.\n\n"
            "⚠️ این قابلیت مخصوص تست نسخه ۴ است و در نسخه ۵ از کل سورس حذف خواهد شد. لینک را عمومی نکنید."
        )
        await query.message.reply_text(text, parse_mode="HTML")
    except Exception:
        await query.message.reply_text("⚠️ ساخت لینک تست ناموفق بود. دوباره تلاش کنید.")


async def admin_referral_test_disable(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    await database.disable_referral_test_link()
    await database.log_action("admin", query.from_user.id, "غیرفعال‌سازی لینک تست رفرال نسخه ۴")
    await query.answer("لینک تست غیرفعال شد.", show_alert=True)
    await admin_referral_settings_show(update, context)


async def admin_referral_test_reset(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    removed = await database.reset_referral_test_balance(query.from_user.id)
    await database.log_action("admin", query.from_user.id, f"ریست موجودی/رکوردهای تست رفرال | {removed}")
    await query.answer(f"{removed:,} تومان موجودی تست ریست شد و رکوردهای تست پاک شدند.", show_alert=True)
    await admin_referral_settings_show(update, context)
...
async def admin_referral_stats(update, context):
    ...
    text = (
        "📊 <b>آمار فوق دقیق رفرال</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        f"👥 کل دعوت‌های ثبت‌شده: <b>{stats['total']}</b>\n"
        f"🛒 دعوت‌های واجدشرایط: <b>{stats['qualified']}</b>\n"
        f"💰 کل پاداش ثبت‌شده: <b>{stats['rewards']:,}</b> تومان\n"
        f"🔒 باقی‌مانده در انتظار: <b>{stats['locked']:,}</b> تومان\n\n"
        "👇 آخرین دعوت‌ها:"
    )
    keyboard = []
    for r in rows:
        rid = r['id']
        name = r.get('referred_username') or str(r.get('referred_id'))
        refname = r.get('referrer_username') or str(r.get('referrer_id'))
        reward = int(r.get('reward_amount') or 0)
        released = int(r.get('released_amount') or 0)
        locked = max(0, reward - released)
        keyboard.append([h.ibtn(
            f"👤 {name} | دعوت‌کننده: {refname} | 🔓{released:,} | 🔒{locked:,}",
            callback_data=f"admin_referral_detail:{rid}"
        )])
    nav = []
    if page > 0:
        nav.append(h.ibtn("⬅️ قبلی", callback_data=f"admin_referral_stats:{page-1}"))
    if len(rows) == 10:
        nav.append(h.ibtn("بعدی ➡️", callback_data=f"admin_referral_stats:{page+1}"))
    if nav:
        keyboard.append(nav)
    keyboard.append([h.ibtn("🔙 بازگشت", callback_data="admin_referral_settings_back")])
...
async def admin_referral_detail(update, context):
    ...
    row = await database.get_referral_admin_detail(rid)
    if not row:
        await query.answer("این رفرال پیدا نشد.", show_alert=True)
        return
    reward = int(row.get('reward_amount') or 0)
    released = int(row.get('released_amount') or 0)
    locked = max(0, reward - released)
    text = (
        "👤 <b>جزئیات رفرال</b>\n━━━━━━━━━━━━━━━━━━\n"
        f"🆔 رکورد: <code>{rid}</code>\n"
        f"👤 دعوت‌شده: <code>{row['referred_id']}</code>\n"
        f"🔗 دعوت‌کننده: <code>{row['referrer_id']}</code>\n"
        f"📅 ثبت: {row.get('created_at') or '-'}\n"
        f"📦 وضعیت: <b>{row.get('status') or '-'}</b>\n"
        f"🛒 واجدشرایط: <b>{'بله' if row.get('qualified') else 'خیر'}</b>\n"
        f"💰 پاداش: <b>{reward:,}</b> تومان\n"
        f"🔓 آزادشده: <b>{released:,}</b> تومان\n"
        f"🔒 در انتظار: <b>{locked:,}</b> تومان"
    )
    keyboard = InlineKeyboardMarkup([
        [h.ibtn("🗑 حذف رکورد رفرال", callback_data=f"admin_referral_reset:{rid}", style="danger")],
        [h.ibtn("🔙 بازگشت به آمار", callback_data="admin_referral_stats:0")],
    ])
    await query.answer()
...
async def admin_referral_reset(update, context):
    ...
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    rid = int(query.data.split(":")[1])
    ok = await database.reset_referral_record(rid)
    await database.log_action("admin", query.from_user.id, f"حذف attribution رفرال #{rid} | نتیجه {ok}")
