...
async def _broadcast_copy_message(context, user_ids, from_chat_id, message_id, reply_markup=None):
    """کپی واقعی پیام برای Broadcast؛ متن، HTML/Markdown entities، کپشن، عکس، ویدئو، GIF، فایل، صدا، استیکر و InlineKeyboard را تا حد ممکن عیناً حفظ می‌کند."""
    sent = 0
    ...
    async def one(uid):
        ...
        async with sem:
            for attempt in range(4):
                try:
                    await context.bot.copy_message(chat_id=uid, from_chat_id=from_chat_id, message_id=message_id, reply_markup=reply_markup)
                    sent += 1
                    return
                except RetryAfter as e:
                    ...
                    # بعضی پیام‌های خاص قابل copy نیستند؛ فوروارد همان پیام را به‌عنوان fallback امتحان می‌کنیم.
                    if attempt == 0:
                        try:
                            ...
                            await asyncio.sleep(float(re.retry_after) + 0.5)
                        except Exception:
                            return
                    return
                except Exception:
    ...
    for i in range(0, len(ids), 20):
        await asyncio.gather(*(one(uid) for uid in ids[i:i+20]), return_exceptions=True)
    return sent
...
async def admin_receive_forward_input(update, context):
    """ورودی عمومی پیام همگانی: هر پیام عادی یا مدیایی را می‌گیرد و با copy_message برای همه می‌فرستد؛ در نتیجه کپشن، فرمت و دکمه‌ها حفظ می‌شوند."""
    message = update.effective_message
    action = context.user_data.get("admin_action")
    ...
    if action == "broadcast_all":
        user_ids = await database.list_all_user_ids()
        log_label = "پیام همگانی"
    else:
        package_id = int(action.split(":")[1])
        user_ids = await database.list_user_ids_for_package(package_id)
        log_label = f"پیام به کاربران پلن #{package_id}"

    sent = await _broadcast_copy_message(context, user_ids, message.chat_id, message.message_id, getattr(message, "reply_markup", None))
    await database.log_action("admin", update.effective_user.id, f"{log_label} به {sent} کاربر (کپی کامل پیام)")
    await message.reply_text(
        f"✅ پیام شما به {sent} کاربر ارسال شد.\n"
        "📎 نوع پیام، عکس/ویدئو/GIF/فایل، کپشن، قالب‌بندی و دکمه‌های موجود تا حد امکان عیناً حفظ شد."
    )
    context.user_data.pop("admin_action", None)
    return ConversationHandler.END
...
async def admin_referral_settings_show(update, context):
    penalty_enabled = await database.get_setting("referral_penalty_enabled", "0") == "1"
    if not await h.require_perm_msg(update, update.effective_user.id, "settings"):
        return
    enabled = await database.get_setting("referral_enabled", "0") == "1"
    condition = await database.get_setting("referral_condition_enabled", "1") == "1"
    spend_enabled = await database.get_setting("referral_spend_enabled", "0") == "1"
    ...
    spend_min_gb = await database.get_setting("referral_spend_min_gb", "10")
    try:
        penalty_extra = int(float(await database.get_setting("referral_penalty_extra_money", "0") or 0))
    except Exception:
        penalty_extra = 0
    try:
        penalty_gb = float(await database.get_setting("referral_penalty_gb", "0") or 0)
    except Exception:
        penalty_gb = 0.0
    try:
        penalty_days = int(float(await database.get_setting("referral_penalty_days", "0") or 0))
    except Exception:
        penalty_days = 0
    stats = await database.get_referral_stats_for_admin()
    try:
        min_gb_text = str(int(float(min_gb))) if float(min_gb).is_integer() else str(min_gb)
    except Exception:
        min_gb_text = str(min_gb)
    text = (
        "👥 <b>تنظیمات رفرال‌گیری</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        f"وضعیت رفرال: {'🟢 فعال' if enabled else '🔴 خاموش'}\n"
        f"شرط آزادسازی: {'🟢 فعال' if condition else '🔴 خاموش'}\n"
        f"💰 پاداش هر دعوت: <b>{reward:,} تومان</b>\n"
        f"📦 حداقل خرید دوست برای آزادسازی: <b>{min_gb_text} گیگ</b>\n"
        f"💳 محدودیت استفاده از پاداش در خرید: {'🟢 فعال' if spend_enabled else '🔴 خاموش'}\n"
        f"📦 حداقل حجم خرید با موجودی دعوت: <b>{float(spend_min_gb):g} گیگ</b>\n\n"
        f"⚠️ جریمه لفت: {'🟢 فعال' if penalty_enabled else '🔴 خاموش'} | 💸 اضافه: <b>{penalty_extra:,} تومان</b> | 📦 <b>{penalty_gb:g} گیگ</b> | 📅 <b>{penalty_days} روز</b>\n\n"
        f"👥 کل دعوت‌های موفق: <b>{stats['total']}</b>\n"
        f"🔓 مجموع آزادشده: <b>{stats['rewards'] - stats['locked']:,} تومان</b>\n"
        f"🔒 مجموع در انتظار: <b>{stats['locked']:,} تومان</b>"
    )
    toggle = "🔴 خاموش کردن رفرال" if enabled else "🟢 روشن کردن رفرال"
    cond_toggle = "🔴 خاموش کردن شرط" if condition else "🟢 روشن کردن شرط"
    keyboard = InlineKeyboardMarkup([
        [h.ibtn(toggle, callback_data="admin_referral_toggle")],
        [h.ibtn(cond_toggle, callback_data="admin_referral_condition_toggle")],
        [h.ibtn("💰 تعیین قیمت هر رفرال", callback_data="admin_set_referral_reward")],
        [h.ibtn("📦 حداقل خرید برای آزادسازی پاداش", callback_data="admin_referral_min_plan")],
        [h.ibtn("💳 محدودیت استفاده از پاداش در خرید", callback_data="admin_referral_spend_toggle")],
        [h.ibtn("📦 حداقل حجم خرید با موجودی دعوت", callback_data="admin_referral_spend_min_plan")],
        [h.ibtn("⚠️ جریمه لفت رفرال: " + ("🟢 فعال" if penalty_enabled else "🔴 خاموش"), callback_data="admin_referral_penalty_toggle")],
        [h.ibtn("💸 مبلغ اضافه جریمه هر لفت", callback_data="admin_set_referral_penalty_extra_money")],
        [h.ibtn("📦 جریمه سرویس — حجم (گیگ)", callback_data="admin_set_referral_penalty_gb")],
        [h.ibtn("📅 جریمه سرویس — مدت (روز)", callback_data="admin_set_referral_penalty_days")],
        [h.ibtn("✏️ ویرایش متن جریمه لفت", callback_data="admin_set_referral_penalty_text")],
        [h.ibtn("📊 آمار فوق دقیق رفرال", callback_data="admin_referral_stats:0")],
        [h.ibtn("✏️ ویرایش پیام دعوت موفق", callback_data="admin_set_referral_success_text")],
        [h.ibtn("✏️ ویرایش متن صفحه رفرال", callback_data="admin_set_referral_page_text")],
    ])
    await _reply(update, text, reply_markup=keyboard)


async def admin_referral_toggle(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    current = await database.get_setting("referral_enabled", "0")
    new_val = "0" if current == "1" else "1"
    await database.set_setting("referral_enabled", new_val)
    await database.log_action("admin", query.from_user.id, f"تغییر وضعیت رفرال به {new_val}")
    await query.answer("وضعیت رفرال تغییر کرد.")
    await admin_referral_settings_show(update, context)


async def admin_referral_condition_toggle(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    current = await database.get_setting("referral_condition_enabled", "1")
    new_val = "0" if current == "1" else "1"
    await database.set_setting("referral_condition_enabled", new_val)
    released = 0
    if new_val == "0":
        released = await database.release_all_locked_referrals()
    await database.log_action("admin", query.from_user.id, f"تغییر شرط رفرال به {new_val} | آزادسازی {released}")
    await query.answer("شرط رفرال تغییر کرد.")
    await admin_referral_settings_show(update, context)



async def admin_referral_spend_toggle(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    current = await database.get_setting("referral_spend_enabled", "0")
    new_val = "0" if current == "1" else "1"
    await database.set_setting("referral_spend_enabled", new_val)
    await database.log_action("admin", query.from_user.id, f"محدودیت استفاده از پاداش رفرال در خرید به {new_val} تغییر کرد")
    await query.answer("تنظیمات محدودیت خرید تغییر کرد.")
    await admin_referral_settings_show(update, context)


async def admin_referral_spend_min_plan_show(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    packages = await database.list_packages(active_only=True)
    if not packages:
        await query.answer("هیچ پلن فعالی وجود ندارد.", show_alert=True)
        return
    keyboard = []
    for p in packages:
        gb = float(p.get("gb") or 0)
        gb_text = str(int(gb)) if gb.is_integer() else str(gb)
        keyboard.append([h.ibtn(
            f"📦 {p.get('name') or 'پلن'} — {gb_text} گیگ — {int(p.get('price') or 0):,} تومان",
            callback_data=f"admin_referral_spend_min_plan_set:{p['id']}"
        )])
    keyboard.append([h.ibtn("🔙 بازگشت", callback_data="admin_referral_settings_back")])
    await query.answer()
    await query.message.reply_text(
        "💳 <b>حداقل حجم خرید با موجودی حاصل از دعوت</b>\n\n"
        "یک پلن را انتخاب کنید؛ وقتی این محدودیت فعال باشد، اگر پرداخت خرید به بخشی از موجودی حاصل از دعوت نیاز داشته باشد، همان حجم و پلن‌های حجیم‌تر قابل خرید خواهند بود. "
        "کاربر همچنان فقط یک موجودی کیف پول می‌بیند.",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(keyboard)
    )


async def admin_referral_spend_min_plan_set(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    package_id = int(query.data.split(":")[1])
    ...
    if not package:
        await query.answer("پلن پیدا نشد.", show_alert=True)
        return
    gb = float(package.get("gb") or 0)
    await database.set_setting("referral_spend_min_gb", str(gb))
    await database.log_action("admin", query.from_user.id, f"حداقل حجم استفاده از موجودی دعوت روی {gb} گیگ تنظیم شد")
    await query.answer("حداقل حجم خرید ذخیره شد.")
    await admin_referral_settings_show(update, context)


async def admin_referral_min_plan_show(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    packages = await database.list_packages(active_only=True)
    ...
    for p in packages:
        gb = p.get("gb") or 0
        gb_text = str(int(gb)) if float(gb).is_integer() else str(gb)
        keyboard.append([h.ibtn(
            f"📦 {p.get('name') or 'پلن'} — {gb_text} گیگ — {int(p.get('price') or 0):,} تومان",
            callback_data=f"admin_referral_min_plan_set:{p['id']}"
        )])
    keyboard.append([h.ibtn("🔙 بازگشت", callback_data="admin_referral_settings_back")])
    await query.answer()
    await query.message.reply_text(
        "📦 <b>حداقل خرید واجدشرایط رفرال</b>\n\n"
        "یک پلن را انتخاب کنید؛ از این به بعد همان حجم و تمام پلن‌های حجیم‌تر واجدشرایط آزادسازی پاداش خواهند بود.",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(keyboard)
    )


async def admin_referral_min_plan_set(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    package_id = int(query.data.split(":")[1])
    ...
    if not package:
        ...
        return
    gb = float(package.get("gb") or 0)
    await database.set_setting("referral_min_gb", str(gb))
    await database.log_action("admin", query.from_user.id, f"حداقل خرید رفرال روی {gb} گیگ تنظیم شد")
    await query.answer("حداقل خرید ذخیره شد.")
    await admin_referral_settings_show(update, context)


async def admin_referral_settings_back(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    await query.answer()
...
async def admin_referral_penalty_toggle(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    current = await database.get_setting("referral_penalty_enabled", "0")
    new_val = "0" if current == "1" else "1"
    await database.set_setting("referral_penalty_enabled", new_val)
    await database.log_action("admin", query.from_user.id, f"جریمه لفت رفرال به {new_val} تغییر کرد")
    await query.answer("تنظیم جریمه تغییر کرد.")
    await admin_referral_settings_show(update, context)


async def admin_referral_stats(update, context):
    query=update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    ...
    stats=await database.get_referral_stats_for_admin()
    tops=await database.get_referral_top_referrers(limit=10)
    text=("📊 <b>آمار فوق دقیق رفرال</b>\n━━━━━━━━━━━━━━━━━━\n"
          f"👥 کل دعوت‌ها: <b>{stats['total']}</b>\n"
          f"🛒 واجدشرایط خرید: <b>{stats['qualified']}</b>\n"
          f"💰 کل پاداش: <b>{stats['rewards']:,}</b> تومان\n"
          f"🔓 آزادشده: <b>{stats['released']:,}</b> تومان\n"
          f"🔒 در انتظار: <b>{stats['locked']:,}</b> تومان\n"
          f"🚪 لفت: <b>{stats['left_count']}</b>\n"
          f"⚠️ جریمه: <b>{stats['penalties']}</b> | 💸 <b>{stats['penalty_money']:,}</b> تومان | 🧾 بدهی: <b>{stats['debts']:,}</b> تومان\n\n"
          "🏆 <b>برترین دعوت‌کنندگان:</b>")
    kb=[]
    for r in tops:
        name=r.get('username') or str(r['referrer_id'])
        kb.append([h.ibtn(f"🤝 {name} | دعوت {r['total']} | موفق {r['qualified']} | لفت {r['left_count']} | جریمه {r['penalized']}", callback_data=f"admin_referral_user:{r['referrer_id']}")])
    kb.append([h.ibtn("📋 لیست آخرین دعوت‌ها", callback_data="admin_referral_rows:0")])
    kb.append([h.ibtn("🔙 بازگشت", callback_data="admin_referral_settings_back")])
    await query.message.reply_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))


async def admin_referral_rows(update, context):
    query=update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"): return
    page=max(0,int(query.data.split(":")[1]))
    rows=await database.get_referral_admin_rows(limit=10, offset=page*10)
    kb=[]
    for r in rows:
        name=r.get('referred_username') or str(r.get('referred_id')); refname=r.get('referrer_username') or str(r.get('referrer_id'))
        flag=("🚪" if r.get('left_at') else "")+("⚠️" if r.get('penalty_status')=='done' else "")
        kb.append([h.ibtn(f"👤 {name} ← {refname} {flag}", callback_data=f"admin_referral_detail:{r['id']}")])
    nav=[]
    if page>0: nav.append(h.ibtn("⬅️ قبلی", callback_data=f"admin_referral_rows:{page-1}"))
    if len(rows)==10: nav.append(h.ibtn("بعدی ➡️", callback_data=f"admin_referral_rows:{page+1}"))
    if nav: kb.append(nav)
    kb.append([h.ibtn("🔙 آمار", callback_data="admin_referral_stats:0")])
    await query.answer(); await query.message.reply_text("📋 <b>آخرین دعوت‌های ثبت‌شده</b>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))


async def admin_referral_user(update, context):
    query=update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"): return
    referrer_id=int(query.data.split(":")[1])
    ov=await database.get_referral_user_overview(referrer_id); rows=await database.list_referrals_for_referrer(referrer_id,100)
    text=(f"🤝 <b>پرونده رفرال</b>\n━━━━━━━━━━━━━━━━━━\n🆔 <code>{referrer_id}</code>\n"
          f"👥 دعوت: <b>{ov['total']}</b> | موفق: <b>{ov['qualified']}</b>\n"
          f"💰 پاداش: <b>{ov['rewards']:,}</b> | آزادشده: <b>{ov['released']:,}</b>\n"
          f"🚪 لفت: <b>{ov['left_count']}</b> | جریمه: <b>{ov['penalties']}</b>")
    kb=[]
    for r in rows[:30]:
        name=r.get('referred_username') or str(r['referred_id']); status="🚪" if r.get('left_at') else ("⚠️" if r.get('penalty_status')=='done' else "✅")
        kb.append([h.ibtn(f"{status} {name} | +{int(r.get('reward_amount') or 0):,}ت", callback_data=f"admin_referral_detail:{r['id']}")])
    kb.append([h.ibtn("♻️ ریست کامل رفرال این کاربر", callback_data=f"admin_referral_user_reset:{referrer_id}", style="danger")])
    kb.append([h.ibtn("🔙 آمار", callback_data="admin_referral_stats:0")])
    await query.answer(); await query.message.reply_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))


async def admin_referral_user_reset(update, context):
    query=update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"): return
    referrer_id=int(query.data.split(":")[1])
    ok=await database.reset_referrals_for_referrer(referrer_id)
    await database.log_action("admin", query.from_user.id, f"ریست کامل رفرال {referrer_id} | {ok}")
    await query.answer("رفرال این کاربر ریست شد." if ok else "کاربر پیدا نشد.", show_alert=True)
    await admin_referral_stats(update, context)


async def admin_referral_detail(update, context):
    query=update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"): return
    rid=int(query.data.split(":")[1]); row=await database.get_referral_admin_detail(rid)
    if not row: await query.answer("این رفرال پیدا نشد.",show_alert=True); return
    reward=int(row.get('reward_amount') or 0); released=int(row.get('released_amount') or 0); locked=max(0,reward-released)
    text=("👤 <b>جزئیات رفرال</b>\n━━━━━━━━━━━━━━━━━━\n"
          f"🆔 رکورد: <code>{rid}</code>\n👤 دعوت‌شده: <code>{row['referred_id']}</code>\n"
          f"🔗 دعوت‌کننده: <code>{row['referrer_id']}</code>\n📅 ثبت: {row.get('created_at') or '-'}\n"
          f"📦 وضعیت: <b>{row.get('status') or '-'}</b>\n🛒 واجدشرایط: <b>{'بله' if row.get('qualified') else 'خیر'}</b>\n"
          f"💰 پاداش: <b>{reward:,}</b> تومان\n🔓 آزادشده: <b>{released:,}</b> تومان\n🔒 در انتظار: <b>{locked:,}</b> تومان\n"
          f"🚪 لفت: <b>{'بله' if row.get('left_at') else 'خیر'}</b>")
    if row.get('left_at'): text += f"\n📢 کانال لفت: <code>{html.escape(str(row.get('left_channel') or '-'))}</code>"
    if row.get('penalty_status')=='done': text += f"\n⚠️ جریمه نقدی: <b>{int(row.get('penalty_amount') or 0):,}</b> تومان\n📦 جریمه سرویس: <b>{float(row.get('penalty_gb') or 0):g} گیگ</b> | <b>{int(row.get('penalty_days') or 0)} روز</b>"
    kb=[[h.ibtn("🗑 حذف رکورد رفرال",callback_data=f"admin_referral_reset:{rid}",style="danger")],[h.ibtn("🔙 آمار",callback_data="admin_referral_stats:0")]]
    await query.answer(); await query.message.reply_text(text,parse_mode="HTML",reply_markup=InlineKeyboardMarkup(kb))


async def admin_referral_reset(update, context):
    query=update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"): return
    rid=int(query.data.split(":")[1]); ok=await database.reset_referral_record(rid)
    await database.log_action("admin", query.from_user.id, f"حذف attribution رفرال #{rid} | {ok}")
    await query.answer("رکورد حذف شد." if ok else "رکورد پیدا نشد.", show_alert=True)
    await admin_referral_stats(update, context)
...
_NUMERIC_INT_ACTIONS = {"admin_set_trial_days", "admin_set_cb_price_gb", "admin_set_cb_price_day", "admin_set_referral_penalty_extra_money", "admin_set_referral_penalty_days"}
_NUMERIC_FLOAT_ACTIONS = {"admin_set_trial_gb", "admin_set_cb_min_gb", "admin_set_cb_max_gb", "admin_set_referral_penalty_gb"}
...
async def admin_receive_input(update, context):
    ...
    if action == "trust_buy_btn_label":
        ...
        await message.reply_text(
            f"✅ لیبل دکمه ذخیره شد: <b>{text.strip()}</b>\n\n"
            "🔗 حالا لینک دکمه را وارد کنید (مثال: https://t.me/yourbot):",
            parse_mode="HTML",
            reply_markup=h.flow_cancel_keyboard()
        )
        return h.AWAITING_ADMIN_INPUT
    ...
    if action == "trust_buy_btn_url":
        ...
        context.user_data.pop("admin_action", None)
        await message.reply_text(
            f"✅ دکمه خرید ذخیره شد.\n"
            f"🔗 لینک: <code>{text.strip()}</code>\n\n"
            "حالا از بخش «📣 کانال اعتماد» می‌توانی پیش‌نمایش ببینی.",
            parse_mode="HTML"
        )
        return ConversationHandler.END
...
async def admin_connection_guide_toggle(update, context):
    ...
    enabled = new_val == "1"
    title = await database.get_setting("connection_guide_title", "📲 آموزش اتصال")
    text_val = await database.get_setting("connection_guide_text", "تنظیم نشده")
    btn1 = await database.get_setting("connection_guide_btn1_label", "")
    url1 = await database.get_setting("connection_guide_btn1_url", "")
    btn2 = await database.get_setting("connection_guide_btn2_label", "")
    url2 = await database.get_setting("connection_guide_btn2_url", "")
    ...
    try:
        await query.message.edit_text(info, parse_mode="HTML", reply_markup=keyboard)
    except Exception:
        await query.message.reply_text(info, parse_mode="HTML", reply_markup=keyboard)
...
async def admin_service_template_select(update, context):
    ...
    text = (
        f"🎨 <b>قالب‌های سرویس</b>\n"
        f"━━━━━━━━━━━━━━━\n"
        f"✅ قالب فعلی: <b>{SERVICE_TEMPLATES[idx]['name']}</b> ({idx+1}/{total})"
    )
    try:
        await query.message.edit_text(text, parse_mode="HTML", reply_markup=_service_template_keyboard(idx))
    except Exception:
...
async def _refresh_trust_menu(query):
    """بازسازی منوی کانال اعتماد با اطلاعات به‌روز"""
    ...
    info = (
        ...
        f"دکمه خرید زیر پیام: {'✅ فعال' if buy_btn_enabled else '❌ غیرفعال'}\n"
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [h.ibtn("🔘 دکمه خرید زیر پیام: " + ("✅ فعال" if buy_btn_enabled else "❌ غیرفعال"),
...
async def admin_trust_toggle(update, context):
    ...
    await _refresh_trust_menu(query)
