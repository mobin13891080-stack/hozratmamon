...
async def admin_generic_setting_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text(prompt + " (لفو: /cancel)")
...
async def admin_welcome_photo_start(update, context):
    ...
    await update.message.reply_text("🖼️ عکس جدید را ارسال کنید، یا 0 را بفرستید تا عکس حذف شود. (لفو: /cancel)")
    return h.AWAITING_ADMIN_INPUT
...
async def admin_pkg_add_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("📦 نام پلن جدید را وارد کنید: (لفو: /cancel)")
    return h.AWAITING_ADMIN_INPUT
...
async def admin_pkg_field_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text(f"{prompts.get(field, field)} را وارد کنید: (لفو: /cancel)")
    return h.AWAITING_ADMIN_INPUT
...
async def admin_admins_add_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("👮 آیدی عددی تلگرام ادمین جدید را وارد کنید: (لفو: /cancel)")
    return h.AWAITING_ADMIN_INPUT
...
async def admin_cancel(update, context):
    ...
    await update.message.reply_text("✅ <b>عملیات لفو شد.</b>", parse_mode="HTML")
    return ConversationHandler.END
...
# ---------------- زیرمنوهای تنطیمات (کارت ، عضویت اجباری، تست رایگان، قوانین، مدیریت) ----------------
...
async def admin_forcejoin_add_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("📢 <b>آیدی/یوزرنیم کانال جدید را و��رد کنید (مثلا @mychannel):</b>", parse_mode="HTML", reply_markup=h.flow_cancel_keyboard())
    return h.AWAITING_ADMIN_INPUT
...
_NUMERIC_INT_ACTIONS = {"admin_set_trial_days"}
_NUMERIC_FLOAT_ACTIONS = {"admin_set_trial_gb"}
...
async def admin_receive_input(update, context):
    ...
    if action == "admin_add_forcejoin_channel":
        ...
        if not (new_channel.startswith("@") or new_channel.startswith("https://") or new_channel.startswith("-100")):
            await message.reply_text("لطفا یوزرنیم کانال (مثلا @mychannel)، لینک دعوت یا آیدی عددی (-100...) وارد کنید.")
            return h.AWAITING_ADMIN_INPUT
        channels = await h.get_force_join_channels()
    ...
    context.user_data.pop("admin_action", None)
    await message.reply_text("متوجه نشدم. /cancel را بزنید و دوباره امتحان کنید.")
    return ConversationHandler.END
