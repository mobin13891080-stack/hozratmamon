...
async def admin_trial_settings_show(update, context):
    ...
    unit = await database.get_setting("trial_period_unit", "day")
    unit_fa = "ساعت" if unit == "hour" else "روز"
    status_icon = "🟢 فعال" if enabled else "🔴 معلق"
    text = (
        f"🎁 تست رایگان\nوضعیت: {status_icon}\nحجم: {gb} گیگ\nمدت: {days} {unit_fa}\n\n"
        "♻️ دکمه «ریست تست» به همه‌ی کاربرانی قبلی یک‌بار دیگر اجازه دریافت دوباره تست می‌دهد."
    )
    toggle_label = "🔴 نیرفعال کردن" if enabled else "🟢 فعال کردن"
    unit_toggle_label = "⏱ سوییچ به ساعتی" if unit != "hour" else "📅 سوییچ به روزانه"
    keyboard = InlineKeyboardMarkup([
        ...
        [h.ibtn(unit_toggle_label, callback_data="admin_trial_unit_toggle")],
        [h.ibtn("♻️ ریست تست (اجازه دوباره به همه)", callback_data="admin_trial_reset_confirm", style="danger")],
    ])
    await _reply(update, text, reply_markup=keyboard)
...
async def admin_trial_toggle(update, context):
    ...
    await safe_answer(query, "وضعیت عوض شد.")
...
async def admin_trial_unit_toggle(update, context):
    ...
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    current = await database.get_setting("trial_period_unit", "day")
    new_unit = "hour" if current != "hour" else "day"
    await database.set_setting("trial_period_unit", new_unit)
    unit_fa = "ساعت" if new_unit == "hour" else "روز"
    await database.log_action("admin", query.from_user.id, f"واحد مدت تست رایگان به {unit_fa} تغییر کرد")
    await safe_answer(query, "واحد مدت تست عوض شد.")
    await admin_trial_settings_show(update, context)


async def admin_trial_reset_confirm(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    await safe_answer(query)
    keyboard = InlineKeyboardMarkup([
        [h.ibtn("✅ بله، ریست کن", callback_data="admin_trial_reset_do", style="danger"),
         h.ibtn("❌ انصراف", callback_data="admin_trial_settings")],
    ])
    await h.safe_edit_or_send(
        query,
        "⚠️ آیا مطمئنید؟ با این کار همه‌ی کاربرانی که تا این لحظه تست رایگان گرفته‌اند، دوباره (فقط برای یک‌بار دیگر تا ریست بعدی) مجاز به گرفتن تست خواهند شد.",
        reply_markup=keyboard,
    )


async def admin_trial_reset_do(update, context):
    query = update.callback_query
    if not await h.require_perm(query, query.from_user.id, "settings"):
        return
    affected = await database.reset_trial_all()
    await database.log_action("admin", query.from_user.id, f"ریست تست رایگان برای {affected} کاربر")
    await safe_answer(query, "✅ ریست شد.")
    await query.message.reply_text(f"✅ تست رایگان ریست شد؛ {affected} کاربر دوباره مجاز به دریافت تست شدند.")
    await admin_trial_settings_show(update, context)
