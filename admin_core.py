...
async def _categories_view():
    ...
    text = (
        "🗂 دسته‌بندی پلن‌ها\n━━━━━━━━━━━━━━━\n"
        "روی نام دسته بزنید تا نامش را ویرایش کنید. با ➕ دسته جدید (مثلاً هفتگی، ماهانه، فصلی) بسازید و از بخش ویرایش هر پلن، دستهش را مشخص کنید."
    )
    if not categories:
...
# ---------------- زیرمنوهای تنطیمات (کارت ، عضویت اجباری، تست رایگان، قوانین، و مدیریت) ----------------
...
async def admin_receive_input(update, context):
    ...
    if action in h.ASYNC_SETTING_ACTIONS:
        ...
        await update.message.reply_text("✅ با م����فقیت ذخیره شد.")
        context.user_data.pop("admin_action", None)
        return ConversationHandler.END
    ...
    if action == "admin_wallet_credit_amount":
        ...
        try:
            await context.bot.send_message(chat_id=target_id, text=f"🎉 مبلف {amount:,} تومان به کیف پول شما افزوده شد توسط مدیریت.\n💰 موجودی جدید: {new_balance:,} تومان")
        except Exception:
            pass
        context.user_data.pop("admin_action", None)
    ...
    if action == "admin_wallet_debit_amount":
        ...
        try:
            await context.bot.send_message(chat_id=target_id, text=f"⚠️ مبلف {amount:,} تومان از کیف پول شما کاسته شد توسط مدیریت.\n💰 موجودی جدید: {new_balance:,} تومان")
        except Exception:
            pass
        context.user_data.pop("admin_action", None)
        context.user_data.pop("wallet_target_user", None)
