...
async def admin_referral_test_link(update, context):
    ...
    try:
        ...
        text = (
            ...
            "━━━━━━━━━━━━━━━━━━\n"
            f"🔗 <code>{link}</code>\n\n"
            "هر اکانتی این لینک را Start کند، یک پاداش تست با مبلغ فعلی رفرال به کیف پول ادمین سازنده اضافه می‌شود؛ حتی اگر قبلاً ربات را Start کرده یا رفرال دیگری داشته باشد.\n\n"
...
async def admin_referral_test_reset(update, context):
    ...
    await database.log_action("admin", query.from_user.id, f"ریست موجودی تست رفرال | {removed}")
    await query.answer(f"{removed:,} تومان موجودی تست ریست شد.", show_alert=True)
    await admin_referral_settings_show(update, context)
