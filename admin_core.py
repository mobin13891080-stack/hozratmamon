...
async def admin_admins_add_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("👮 آیدی عددی تلگرام ادمین جدید را وارد کنید:", reply_markup=h.flow_cancel_keyboard())
    return h.AWAITING_ADMIN_INPUT
...
async def admin_forcejoin_add_start(update, context):
    ...
    await query.answer()
    await query.message.reply_text("📢 <b>آیدی/یوزرنیم کانال جدید را وارد کنید (مثلا @mychannel):</b>", parse_mode="HTML", reply_markup=h.flow_cancel_keyboard())
    return h.AWAITING_ADMIN_INPUT
